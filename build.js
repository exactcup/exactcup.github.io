...
function categoryPage(key) {
  ...
  const description = key === "sugar"
    ...
    : key === "grain"
    ? `Free grain conversion chart: grams per cup for rice, quinoa, oats, couscous, bulgur and pearl barley — plus every cup fraction (half a cup of rice is ${g2(w("white-rice") / 2)} g), a 100 g reverse lookup, and how to sanity-check a grain weight that looks wrong.`
    : key === "flour"
...
<p class="note">One habit removes all of this: weigh the grain dry, before rinsing. Rinsed rice holds surface and absorbed water &mdash; soaking can add around 20% &mdash; so a drained cup is no longer ${g2(w("white-rice"))}&nbsp;g. Comparing two grains rather than checking one? The <a href="/ingredient-substitution-chart/">ingredient substitution chart</a> works out how much of a replacement grain to measure.</p>` : ""}${key === "baking" ? `
