...
// Oat flour cup weights, verified 2026-09-04. King Arthur ingredient chart: oat flour
// 3 1/4 oz = 92 g (this page). USDA SR Legacy #169741 "Oat flour, partially debranned":
// 1 cup = 104 g; per 100 g 404 kcal / 14.66 g protein / 9.12 g fat / 6.5 g fiber.
// Labels: King Arthur retail 1/3 cup = 28 g; Arrowhead Mills + Gold Medal 1/3 cup = 30 g;
// Bob's Red Mill + Quaker 1/4 cup = 30 g. Quaker "Oat Flour 101": swap for AP flour 1:1 by
// weight, or 1 1/4 cups oat flour per cup by volume; only ~1/3 of the flour in bakes that rise.
const OAT_FLOUR_USDA_CUP = 104;
const OAT_FLOUR_100 = { kcal: 404, protein: 14.66, fat: 9.12, fiber: 6.5 };
const OAT_FLOUR_SOURCES = [
  ["Oat flour — King Arthur ingredient chart, spooned (this page)", { slug: "oat-flour" }],
  ["Oat flour — USDA FoodData Central, partially debranned (#169741)", OAT_FLOUR_USDA_CUP],
  ["Oat flour — King Arthur retail label (1/3 cup = 28 g)", 28 * 3],
  ["Oat flour — Arrowhead Mills and Gold Medal labels (1/3 cup = 30 g)", 30 * 3],
  ["Oat flour — Bob's Red Mill and Quaker labels (1/4 cup = 30 g)", 30 * 4],
  ["Rolled oats, for comparison", { slug: "rolled-oats" }],
  ["All-purpose flour, for comparison", { slug: "all-purpose-flour" }],
];
function oatFlourSourcesTable() {
  const rows = OAT_FLOUR_SOURCES.map(([label, w]) => {
    const g = typeof w === "number" ? w : ingBySlug(w.slug).gramsPerCup;
    const c = cups2(100 / g);
    return `<tr><td>${label}</td><td class="num">${g2(g)} g</td><td class="num">${g2(g / 2)} g</td><td class="num">${g2(g / 4)} g</td><td class="num">${c} cup${c === 1 ? "" : "s"}</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Source</th><th>1 US cup</th><th>1/2 cup</th><th>1/4 cup</th><th>100 g =</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
const OAT_FLOUR_ML = [
  [59.147, "1/4 US cup"], [78.863, "1/3 US cup"], [100, "100 mL"], [118.294, "1/2 US cup"],
  [125, "125 mL (half a metric cup)"], [160, "160 mL"], [180, "180 mL (rice-cooker cup)"],
  [200, "200 mL (a standard drinking glass)"], [236.588, "1 US cup"], [250, "250 mL (metric cup: UK, Australia, NZ)"],
];
function oatFlourMlTable(gpc) {
  const rows = OAT_FLOUR_ML.map(([ml, label]) => {
    const g = gpc / 236.588 * ml;
    return `<tr><td>${label}</td><td class="num">${g2(ml)} mL</td><td class="num">${g2(g)} g</td><td class="num">${g2(g * OAT_FLOUR_USDA_CUP / gpc)} g</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Measure</th><th>Millilitres</th><th>Oat flour (this page)</th><th>Oat flour (USDA cup)</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
const OAT_FLOUR_SWAP = [[0.25, "1/4 cup"], [1 / 3, "1/3 cup"], [0.5, "1/2 cup"], [2 / 3, "2/3 cup"], [0.75, "3/4 cup"], [1, "1 cup"], [1.5, "1 1/2 cups"], [2, "2 cups"]];
function oatFlourSwapTable(gpc, apg) {
  const rows = OAT_FLOUR_SWAP.map(([c, label]) => {
    const ap = apg * c;
    return `<tr><td>${label} all-purpose flour</td><td class="num">${g2(ap)} g</td><td class="num">${g2(ap)} g</td><td class="num">${cups2(ap / gpc)} cups</td><td class="num">${g2(gpc * c)} g</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Recipe calls for</th><th>That is</th><th>Oat flour to weigh</th><th>Oat flour by volume</th><th>If you swap cup-for-cup you get only</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}

// Rolled and steel-cut oats, verified 2026-09-05. Rolled: King Arthur chart "Oats
// (old-fashioned or quick-cooking)" 3 1/8 oz = 89 g (this site uses 90); USDA SR
// Legacy #173904 dry 1 cup = 81 g, per 100 g 379 kcal / 13.15 g protein / 6.52 g fat
// / 10.1 g fiber; #173905 cooked with water 1 cup = 234 g, 71 kcal per 100 g. Labels:
// Quaker Old Fashioned and Quick 1/2 cup = 40 g (USDA Branded #1410598, SR #172989);
// Bob's Red Mill old fashioned 1/2 cup = 49 g, quick 45 g, extra thick 50 g; King
// Arthur "Oats (King Arthur Rolled)" 113 g. Steel-cut: Quaker label 1/4 cup = 40 g
// (USDA Branded #2285250; this site's 160 g/cup), Whole Foods / Publix / Harris
// Teeter / Market Basket 1/4 cup = 40 g, Bob's Red Mill 1/4 cup = 44 g (#2014733),
// Arrowhead Mills (Hain #2121423) and Rocky Mountain 1/3 cup = 45 g, King Arthur
// chart 1/2 cup = 70 g; USDA Foundation #2346397 lists no cup measure. Cooking:
// Quaker rolled 1/2 cup + 1 cup water, 5 min (no yield stated); Quaker steel-cut
// 1/4 cup + 1 1/2 cups water, 25-30 min (no yield stated); Bob's Red Mill steel-cut
// 1 cup + 3 cups water "yields about 3 cups". Serve definitions: eatforhealth.gov.au
// (adopted by the NZ Ministry of Health) 1 standard serve = 1/2 cup (120 g) cooked
// porridge or 1/4 cup (30 g) muesli, each ~500 kJ; USDA MyPlate 1 ounce-equivalent
// = 1/2 cup cooked oatmeal, 1 packet instant, or 1 oz (1/3 cup) dry; Whole Grains
// Council 16 g whole grain = 1 serving (8 g = half). Rice-cooker cup (go) = 180 mL.
const OATS_USDA_CUP = 81;
const OATS_COOKED_CUP = 234;
const OATS_COOKED_FACTOR = 5.35; // shared with /dry-to-cooked/
const OATS_100 = { kcal: 379, protein: 13.15, fat: 6.52, fiber: 10.1 };
const OATS_COOKED_100_KCAL = 71;
const OATS_SERVE_AU_G = 30;  // dry oats per AU/NZ ~500 kJ standard serve (30 g = 476 kJ at USDA's 1586 kJ/100 g)
const OATS_SERVE_WGC_G = 16;
const OATS_LABELS = { quakerHalfCup: 40, bobsHalfCup: 49, bobsQuickHalfCup: 45, bobsThickHalfCup: 50 };
const RICE_CUP_ML = 180;
const OATS_ML = [
  [100, "100 mL"], [118.294, "1/2 US cup"], [125, "125 mL (half a metric cup)"], [RICE_CUP_ML, "180 mL (rice-cooker cup)"],
  [200, "200 mL (a standard drinking glass)"], [236.588, "1 US cup"], [250, "250 mL (metric cup: UK, Australia, NZ)"],
  [500, "500 mL"], [1000, "1 litre"],
];
function oatsMlTable(gpc, scg) {
  const rows = OATS_ML.map(([ml, label]) => {
    const f = ml / 236.588;
    return `<tr><td>${label}</td><td class="num">${g2(ml)} mL</td><td class="num">${g2(gpc * f)} g</td><td class="num">${g2(OATS_USDA_CUP * f)} g</td><td class="num">${g2(scg * f)} g</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Measure</th><th>Millilitres</th><th>Rolled oats (this page)</th><th>Rolled oats (USDA cup)</th><th>Steel-cut oats</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
const OATS_SERVE_CUPS = [[0.25, "1/4 cup"], [1 / 3, "1/3 cup"], [0.5, "1/2 cup"], [2 / 3, "2/3 cup"], [1, "1 cup"], [4 / 3, "1 1/3 cups"], [1.5, "1 1/2 cups"], [2, "2 cups"]];
function oatsServesTable(gpc) {
  const rows = OATS_SERVE_CUPS.map(([c, label]) => {
    const g = gpc * c;
    return `<tr><td>${label} dry oats</td><td class="num">${g2(g)} g</td><td class="num">${(g / OATS_SERVE_WGC_G).toFixed(1)}</td><td class="num">${(g / OZ).toFixed(1)}</td><td class="num">${(g / OATS_SERVE_AU_G).toFixed(1)}</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Dry rolled oats</th><th>Grams (= grams of whole grain)</th><th>Whole Grains Council servings (16 g)</th><th>USDA ounce-equivalents (1 oz dry)</th><th>Australian / NZ standard serves (~30 g dry)</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
const STEEL_CUT_KA_CUP = 140;
const STEEL_CUT_BOBS_CUP = 176;
const STEEL_CUT_COOKED_CUP = 245; // derived: Bob's 176 g oats + 710 g water -> "about 3 cups" after ~150 g of steam; USDA cooked oatmeal is 234
const STEEL_CUT_SOURCES = [
  ["Quaker Steel Cut Oats label (1/4 cup = 40 g) — this page", { slug: "steel-cut-oats" }],
  ["Whole Foods, Publix, Harris Teeter, Market Basket labels (1/4 cup = 40 g)", 160],
  ["King Arthur ingredient chart (1/2 cup = 70 g)", STEEL_CUT_KA_CUP],
  ["Arrowhead Mills and Rocky Mountain labels (1/3 cup = 45 g)", 135],
  ["Bob's Red Mill label (1/4 cup = 44 g)", STEEL_CUT_BOBS_CUP],
  ["Rolled oats, for comparison", { slug: "rolled-oats" }],
];
function steelCutSourcesTable() {
  const rows = STEEL_CUT_SOURCES.map(([label, w]) => {
    const g = typeof w === "number" ? w : ingBySlug(w.slug).gramsPerCup;
    const c = cups2(100 / g);
    return `<tr><td>${label}</td><td class="num">${g2(g)} g</td><td class="num">${g2(g / 2)} g</td><td class="num">${g2(g / 4)} g</td><td class="num">${c} cup${c === 1 ? "" : "s"}</td></tr>`;
  }).join("");
...
const STEEL_CUT_ML = [
  [59.147, "1/4 US cup"], [100, "100 mL"], [118.294, "1/2 US cup"], [RICE_CUP_ML, "180 mL (rice-cooker cup)"],
  [200, "200 mL (a standard drinking glass)"], [236.588, "1 US cup"], [250, "250 mL (metric cup: UK, Australia, NZ)"],
  [500, "500 mL"], [1000, "1 litre"],
];
function steelCutMlTable(gpc) {
  const rows = STEEL_CUT_ML.map(([ml, label]) => {
    const f = ml / 236.588;
    return `<tr><td>${label}</td><td class="num">${g2(ml)} mL</td><td class="num">${g2(gpc * f)} g</td><td class="num">${g2(STEEL_CUT_KA_CUP * f)} g</td><td class="num">${g2(STEEL_CUT_BOBS_CUP * f)} g</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Measure</th><th>Millilitres</th><th>Steel-cut (this page, ${gpc} g/cup)</th><th>King Arthur (${STEEL_CUT_KA_CUP} g/cup)</th><th>Bob's Red Mill (${STEEL_CUT_BOBS_CUP} g/cup)</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}

// Quinoa cup weights, verified 2026-09-03. Dry white quinoa: USDA SR Legacy
// #168874 measures 1 cup = 170 g (this site's value); King Arthur's chart lists
// "Quinoa (whole)" at 6 1/4 oz = 177 g. Brand labels bracket both: Bob's Red
// Mill white and tri-color print 1/4 cup = 47 g, Ancient Harvest traditional
// and Inca Red 1/4 cup = 45 g (Trader Joe's 42 g and Kirkland 44 g appear only
// in third-party listings, so they are mentioned as a range, not tabled).
// Cooked: USDA #168917 1 cup = 185 g (King Arthur 184 g). Flakes: Ancient
// Harvest label 1/3 cup = 40 g. Flour: King Arthur 110 g (Bob's Red Mill label
// 1/4 cup = 28 g). Nutrition per 100 g from the same two USDA entries. Cooked
// weight factor 3.06 is the /dry-to-cooked/ figure so the pages agree.
const QUINOA_KA_CUP = 177;
const QUINOA_COOKED_CUP = 185;
const QUINOA_COOKED_FACTOR = 3.06;
const QUINOA_DRY_100 = { kcal: 368, protein: 14.12, fiber: 7.0 };
const QUINOA_COOKED_100 = { kcal: 120, protein: 4.4, fiber: 2.8 };
const QUINOA_TYPES = [
  ["White quinoa, dry — USDA (this page)", { slug: "quinoa" }],
  ["White quinoa, dry — King Arthur chart", QUINOA_KA_CUP],
  ["White quinoa, dry — Ancient Harvest label (1/4 cup = 45 g)", 45 * 4],
  ["White quinoa, dry — Bob's Red Mill label (1/4 cup = 47 g)", 47 * 4],
  ["Red quinoa, dry — Ancient Harvest Inca Red label (1/4 cup = 45 g)", 45 * 4],
  ["Tri-color quinoa, dry — Bob's Red Mill label (1/4 cup = 47 g)", 47 * 4],
  ["Cooked quinoa — USDA", QUINOA_COOKED_CUP],
  ["Quinoa flakes — Ancient Harvest label (1/3 cup = 40 g)", 40 * 3],
  ["Quinoa flour — King Arthur chart", 110],
];
function quinoaTypesTable() {
  const rows = QUINOA_TYPES.map(([label, w]) => {
    const g = typeof w === "number" ? w : ingBySlug(w.slug).gramsPerCup;
    const c = cups2(100 / g);
    return `<tr><td>${label}</td><td class="num">${g2(g)} g</td><td class="num">${g2(g / 2)} g</td><td class="num">${g2(g / 4)} g</td><td class="num">${c} cup${c === 1 ? "" : "s"}</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Quinoa, as sold</th><th>1 US cup</th><th>1/2 cup</th><th>1/4 cup</th><th>100 g =</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
const QUINOA_CUP_SIZES = [
  [100, "100 mL"], [125, "125 mL (half a metric cup)"], [160, "160 mL"], [180, "180 mL (rice-cooker cup)"],
  [200, "200 mL (a standard drinking glass)"], [236.588, "236.6 mL (US cup)"], [250, "250 mL (metric cup: UK, Australia, NZ)"],
];
function quinoaCupSizesTable(gpc) {
  const rows = QUINOA_CUP_SIZES.map(([ml, label]) => {
    const g = gpc / 236.588 * ml;
    return `<tr><td>${label}</td><td class="num">${g2(g)} g</td><td class="num">${g2(g / 2)} g</td><td class="num">${Math.round(g * QUINOA_COOKED_FACTOR)} g</td></tr>`;
  }).join("");
  return `<table><thead><tr><th>Cup or glass</th><th>Dry quinoa</th><th>Half of it</th><th>Cooked weight</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
// Grated parmesan cup weights, verified 2026-09-06. King Arthur ingredient chart: "Cheese
// (grated Parmesan)" 1/2 cup = 1 3/4 oz = 50 g (this page's 100 g/cup). USDA SR Legacy #171247
// "Cheese, parmesan, grated": 1 cup = 100 g, 1 tbsp = 5 g (the two USDA portions do not
// reconcile: 16 x 5 = 80); per 100 g 420 kcal / 28.42 g protein / 27.84 g fat / 1804 mg sodium /
// 853 mg calcium. USDA #173431 shredded: 1 tbsp = 5 g; 415 kcal / 37.86 g protein / 1696 mg
// sodium / 1253 mg calcium. USDA #170848 hard: 1 cubic inch = 10.3 g; 392 kcal / 35.75 g protein.
// Labels: Kraft Grated Parmesan shaker 2 tsp = 5 g (-> 120 g/cup); Kraft Finely Shredded and
// Sargento Fine Cut 1/4 cup = 28 g (-> 112); BelGioioso grated + shredded and 4C grated
// 1 tbsp = 5 g (-> 80). America's Test Kitchen "Grating Cheese Weight and Volume": 1 oz hard
// cheese = 1/2 cup rasp-grated, 1/3 cup medium box grater, 1/4 cup coarse box grater.
// Parmigiano Reggiano PDO specification: minimum 12 months' maturation; grated product
// moisture 25-35%, rind not more than 18% by weight; grating must happen in the area of origin.
const PARM_USDA_CUP = 100;
const PARM_USDA_TBSP = 5;
const PARM_KRAFT_SHAKER_CUP = 5 / 2 * 48; // 2 tsp = 5 g
const PARM_LABEL_QUARTER_CUP = 28; // Kraft Finely Shredded, Sargento Fine Cut
const PARM_100 = { kcal: 420, protein: 28.42, fat: 27.84, sodium: 1804, calcium: 853 };
const PARM_SHRED_100 = { kcal: 415, protein: 37.86, sodium: 1696, calcium: 1253 };
const PARM_HARD_100 = { kcal: 392, protein: 35.75, sodium: 1175, calcium: 1184 };
const PARM_GRATES = [
  ["Microplane / rasp grater (fine, fluffy)", 0.5],
  ["Box grater, medium holes", 1 / 3],
  ["Box grater, coarse holes (shredded)", 0.25],
];
const parmGrateCup = (cupsPerOz) => OZ / cupsPerOz;
const PARM_SOURCES = [
  ["Grated parmesan &mdash; King Arthur ingredient chart, 1/2 cup = 1 3/4 oz = 50 g (this page)", { slug: "grated-parmesan" }],
  ["Grated parmesan &mdash; USDA FoodData Central measured cup (#171247)", PARM_USDA_CUP],
  ["Grated parmesan &mdash; USDA tablespoon (5 g) &times; 16", PARM_USDA_TBSP * 16],
  ["Kraft Grated Parmesan shaker label (2 tsp = 5 g)", PARM_KRAFT_SHAKER_CUP],
  ["Kraft Finely Shredded and Sargento Fine Cut labels (1/4 cup = 28 g)", PARM_LABEL_QUARTER_CUP * 4],
  ["BelGioioso and 4C grated labels (1 tbsp = 5 g)", 5 * 16],
  ["Wedge grated on a Microplane &mdash; America's Test Kitchen (1 oz = 1/2 cup)", parmGrateCup(0.5)],
  ["Wedge on a medium box grater &mdash; ATK (1 oz = 1/3 cup)", parmGrateCup(1 / 3)],
  ["Wedge on a coarse box grater &mdash; ATK (1 oz = 1/4 cup)", parmGrateCup(0.25)],
  ["Shredded cheddar, for comparison", { slug: "shredded-cheddar" }],
];
function parmSourcesTable() {
  const rows = PARM_SOURCES.map(([label, w]) => {
    const g = typeof w === "number" ? w : ingBySlug(w.slug).gramsPerCup;
    return `<tr><td>${label}</td><td class="num">${g2(g)} g</td><td class="num">${g2(g / 2)} g</td><td class="num">${g2(g / 4)} g</td><td class="num">${g2(g / 16)} g</td><td class="num">${cups2(100 / g)} cups</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Source / grate</th><th>1 US cup</th><th>1/2 cup</th><th>1/4 cup</th><th>1 tbsp</th><th>100 g =</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
const PARM_WEDGES = [[OZ, "1 oz"], [2 * OZ, "2 oz"], [100, "100 g"], [4 * OZ, "4 oz (113 g)"], [5 * OZ, "5 oz (142 g)"], [200, "200 g"], [8 * OZ, "8 oz (227 g)"], [16 * OZ, "1 lb (454 g)"]];
function parmWedgeTable() {
  const rows = PARM_WEDGES.map(([g, label]) => {
    const cells = PARM_GRATES.map(([, cpo]) => `<td class="num">${cups2(g / OZ * cpo)} cups</td>`).join("");
    return `<tr><td>${label} wedge</td>${cells}<td class="num">${cups2(g / PARM_USDA_CUP)} cups</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Parmesan you start with</th><th>Microplane (1/2 cup per oz)</th><th>Medium grater (1/3 cup per oz)</th><th>Coarse grater (1/4 cup per oz)</th><th>At this page's 100 g/cup</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
const PARM_ML = [
  [14.787, "1 US tablespoon"], [59.147, "1/4 US cup"], [78.863, "1/3 US cup"], [100, "100 mL"], [118.294, "1/2 US cup"],
  [125, "125 mL (half a metric cup)"], [177.441, "3/4 US cup"], [200, "200 mL"], [236.588, "1 US cup"], [250, "250 mL (metric cup: UK, Australia, NZ)"],
];
function parmMlTable(gpc) {
  const rows = PARM_ML.map(([ml, label]) => {
    const f = ml / 236.588;
    return `<tr><td>${label}</td><td class="num">${g2(ml)} mL</td><td class="num">${g2(gpc * f)} g</td><td class="num">${g2(PARM_KRAFT_SHAKER_CUP * f)} g</td><td class="num">${g2(parmGrateCup(0.5) * f)} g</td></tr>`;
  }).join("");
  return `<div class="tw"><table><thead><tr><th>Measure</th><th>Millilitres</th><th>Grated (this page)</th><th>Kraft shaker</th><th>Microplane fluffy</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
function ingredientPage(ing) {
  ...
  const description = ing.slug === "butter"
    ...
    ? `How many grams is a cup of oats? 1 cup = ${g2(gpc)} g, 1/2 cup = ${g2(gpc / 2)} g — rolled or quick. A 250 mL metric (NZ/AU) cup = ${Math.round(gpc / 236.588 * 250)} g. A 180 mL rice cup = ${Math.round(gpc / 236.588 * 180)} g. Full chart plus whole-grain serves, jumbo, porridge and steel-cut oat weights.`
    : ing.slug === "steel-cut-oats"
    ? `How many grams is a cup of steel-cut oats? 1 cup = ${g2(gpc)} g (Quaker label; King Arthur ${STEEL_CUT_KA_CUP} g, Bob's Red Mill ${STEEL_CUT_BOBS_CUP} g), 1/4 cup = ${g2(gpc / 4)} g, a 180 mL rice cup = ${Math.round(gpc / 236.588 * 180)} g. Full chart plus mL, kilogram and cooked weights.`
    : ing.slug === "cornstarch"
    ...
    : ing.slug === "quinoa"
    ? `How many grams is a cup of quinoa? 1 cup dry = ${g2(gpc)} g (USDA; King Arthur says ${QUINOA_KA_CUP} g), 1/2 cup = ${g2(gpc / 2)} g, cooked = ${QUINOA_COOKED_CUP} g per cup. Full chart plus red and tri-color, flakes, flour, 160–250 mL cups and cooked yield.`
    : ing.slug === "oat-flour"
    ? `How many grams is a cup of oat flour? 1 cup = ${g2(gpc)} g (King Arthur; USDA measures ${OAT_FLOUR_USDA_CUP} g), 1/2 cup = ${g2(gpc / 2)} g, 1 1/2 cups = ${g2(gpc * 1.5)} g. Full chart plus mL, label servings and the by-weight swap for all-purpose flour.`
    : ing.slug === "grated-parmesan"
    ? `How many grams is a cup of grated parmesan? 1 cup = ${g2(gpc)} g (King Arthur and USDA), 1/2 cup = ${g2(gpc / 2)} g, 1/4 cup = ${g2(gpc / 4)} g, 1 tbsp = 5–${g2(gpc / 16)} g. But a microplaned cup is only ~${Math.round(parmGrateCup(0.5))} g and the Kraft shaker ${PARM_KRAFT_SHAKER_CUP} g — full chart by grate size, label servings, mL and calories.`
    : ing.slug === "yogurt"
  ...
  if (ing.slug === "rolled-oats") {
    const scg = ingBySlug("steel-cut-oats").gramsPerCup;
    const riceCup = gpc / 236.588 * RICE_CUP_ML;
    const third = gpc * 4 / 3;
    const dryPerCookedCup = OATS_COOKED_CUP / OATS_COOKED_FACTOR;
    faq.push(
      [`How many grams of oats fit in a rice cup?`, `About ${g2(riceCup)} g of rolled oats. The cup that comes with a rice cooker is a Japanese go of ${RICE_CUP_ML} mL — three-quarters of a US cup (236.6 mL) — so it holds three-quarters of this page's ${g2(gpc)} g: ${g2(riceCup)} g, or ${g2(OATS_USDA_CUP / 236.588 * RICE_CUP_ML)} g by USDA's lighter 81 g cup, and about ${g2(scg / 236.588 * RICE_CUP_ML)} g of steel-cut oats. Quaker's 1/2-cup (40 g) serving is ${cups2(118.294 / RICE_CUP_ML)} of a rice cup, so a level rice cup of Quaker oats is a generous serving and a half. Don't use the cooker's rice lines for the water, though — a rice cup of white rice weighs ${g2(ingBySlug("white-rice").gramsPerCup / 236.588 * RICE_CUP_ML)} g, twice the oats, and oats want roughly twice their volume in water.`],
      [`What is the density of dry oats — how many grams per 100 mL?`, `About ${Math.round(gpc / 2.36588)} g per 100 mL for rolled oats, or ${(gpc / 236.588).toFixed(2)} g/mL (${g2(gpc)} g ÷ 236.6 mL). USDA's measured cup gives ${(OATS_USDA_CUP / 236.588).toFixed(2)} g/mL (${Math.round(OATS_USDA_CUP / 2.36588)} g per 100 mL) and Bob's Red Mill's label works out to ${(OATS_LABELS.bobsHalfCup * 2 / 236.588).toFixed(2)}, so any 100 mL of flakes is ${Math.round(OATS_USDA_CUP / 2.36588)}–${Math.round(OATS_LABELS.bobsHalfCup * 2 / 2.36588)} g. Steel-cut oats are nearly twice as dense at ${(scg / 236.588).toFixed(2)} g/mL (${Math.round(scg / 2.36588)} g per 100 mL), and cooked oatmeal is ${(OATS_COOKED_CUP / 236.588).toFixed(2)} g/mL — essentially water.`],
      [`How many serves of grains is in 1 cup and 1/3 of oats?`, `About ${Math.round(third / OATS_SERVE_AU_G)} serves. One and a third cups of dry rolled oats is ${g2(third)} g, and an Australian or New Zealand standard serve of grain food works out to about ${OATS_SERVE_AU_G} g of dry oats (the guideline's own examples are 1/2 cup / 120 g of cooked porridge or 1/4 cup / 30 g of muesli, each about 500 kJ), so ${g2(third)} ÷ ${OATS_SERVE_AU_G} = ${(third / OATS_SERVE_AU_G).toFixed(1)}. In US terms it is ${(third / OZ).toFixed(1)} ounce-equivalents (1 oz of dry oats each), and under the Whole Grains Council's 16 g rule it is ${(third / OATS_SERVE_WGC_G).toFixed(1)} whole-grain servings. That is a lot of oats: 1 1/3 cups dry cooks up to about ${cups2(third * OATS_COOKED_FACTOR / OATS_COOKED_CUP)} cups of porridge, two to three breakfasts.`],
      [`How many grams of whole grain are in 1/2 cup of uncooked oats?`, `All of it — about ${g2(gpc / 2)} g of whole grain in a half cup (${g2(gpc / 2)} g) of rolled oats, because oats are 100% whole grain: the flake is the entire groat, bran and germ included. Labels print "40 g of whole grain per serving" because the serving Quaker weighs is 1/2 cup = 40 g; use 40 g if you measure like the label, ${g2(gpc / 2)} g if you level a US cup the way this chart assumes. Either way that is ${(40 / OATS_SERVE_WGC_G).toFixed(1)}–${(gpc / 2 / OATS_SERVE_WGC_G).toFixed(1)} Whole Grains Council servings (16 g each), or about ${(gpc / 2 / OZ).toFixed(1)} US ounce-equivalents.`],
      [`How much dry oats makes 1 cup of cooked oatmeal?`, `About ${g2(dryPerCookedCup)} g, or a rounded half cup of dry rolled oats. USDA weighs a cup of oatmeal cooked with water at ${OATS_COOKED_CUP} g, and oats gain about ${OATS_COOKED_FACTOR}× their weight in the pot (Quaker's 1/2 cup oats + 1 cup water cooks down to close to a cup), so the dry equivalent is ${OATS_COOKED_CUP} ÷ ${OATS_COOKED_FACTOR}. Cooked grains are not interchangeable by dry weight: a cup of cooked white rice comes from about ${g2(158 / 2.8)} g of dry rice because rice only gains 2.8×. The <a href="/dry-to-cooked/">dry-to-cooked converter</a> has the factor for every grain.`],
      [`45 grams of oats is how many cups?`, `Half a US cup exactly on this page's ${g2(gpc)} g figure (45 ÷ ${g2(gpc)} = ${cups2(45 / gpc)}). By Quaker's label (40 g per 1/2 cup) 45 g is a slightly heaped half cup, and by Bob's Red Mill's (${OATS_LABELS.bobsHalfCup} g) a slightly short one, so "half a cup" is the right answer for any brand — it is also exactly what Bob's Red Mill prints for a half-cup serving of its quick oats, and one and a half Australian standard serves.`],
      [`How many grams is 1 1/2 cups or 2 1/2 cups of rolled oats?`, `1 1/2 cups is ${g2(gpc * 1.5)} g and 2 1/2 cups is ${g2(gpc * 2.5)} g of old-fashioned or quick oats — multiply ${g2(gpc)} g by the number of cups (2 cups = ${g2(gpc * 2)} g, 3 cups = ${g2(gpc * 3)} g). Measured with Quaker's 40 g half-cup those would read ${OATS_LABELS.quakerHalfCup * 3} g and ${OATS_LABELS.quakerHalfCup * 5} g, so for a big batch of granola or an oat bake the cup matters more than for a bowl of porridge: a 10% cup-to-cup difference on 2 1/2 cups is ${g2(gpc * 0.25)} g, a full quarter cup of oats.`],
      [`How many grams is 1/2 cup of Bob's Red Mill rolled oats?`, `${OATS_LABELS.bobsHalfCup} g by the label on Bob's Red Mill Old Fashioned Rolled Oats (a 1/2-cup serving), ${OATS_LABELS.bobsQuickHalfCup} g for its quick-cooking oats and ${OATS_LABELS.bobsThickHalfCup} g for its extra thick rolled oats — against ${g2(gpc / 2)} g for a half cup on this chart and 40 g on Quaker's label. Bob's larger, thicker flakes settle heavier in the cup, so if a recipe was written with Bob's in mind and you scoop, expect the heavy end of the ${OATS_LABELS.quakerHalfCup * 2}–${OATS_LABELS.bobsThickHalfCup * 2} g-per-cup range; weighing removes the question.`],
      [`How many calories are in a cup of dry oats?`, `About ${Math.round(gpc * OATS_100.kcal / 100)} kcal in a level US cup (${g2(gpc)} g), using USDA's ${OATS_100.kcal} kcal per 100 g for dry rolled or quick oats, with ${g2(gpc * OATS_100.protein / 100)} g of protein, ${g2(gpc * OATS_100.fat / 100)} g of fat and ${g2(gpc * OATS_100.fiber / 100)} g of fiber. Half a cup is about ${Math.round(gpc / 2 * OATS_100.kcal / 100)} kcal — Quaker's label puts its 40 g half-cup at 150 kcal, the same 375 kcal per 100 g — and a cup of cooked oatmeal (${OATS_COOKED_CUP} g, USDA) is only about ${Math.round(OATS_COOKED_CUP * OATS_COOKED_100_KCAL / 100)} kcal, because most of it is water.`],
    );
  }
  if (ing.slug === "steel-cut-oats") {
    const rolled = ingBySlug("rolled-oats").gramsPerCup;
    const riceCup = gpc / 236.588 * RICE_CUP_ML;
    const cookedFactor = STEEL_CUT_COOKED_CUP * 3 / STEEL_CUT_BOBS_CUP;
    faq.push(
      [`How many grams is 1/4 cup of cooked steel-cut oats?`, `About 60 g. Cooked steel-cut porridge is roughly water-density: USDA weighs cooked oatmeal at ${OATS_COOKED_CUP} g per cup (${g2(OATS_COOKED_CUP / 4)} g per quarter cup), and Bob's Red Mill's method — 1 cup of oats to 3 cups of water, "yields about 3 cups" — works out to about ${STEEL_CUT_COOKED_CUP} g per cup (${g2(STEEL_CUT_COOKED_CUP / 4)} g per quarter cup) once the pot has lost some steam. That quarter cup of porridge holds only about ${g2(STEEL_CUT_BOBS_CUP / 12)} g of dry steel-cut oats, a rounded tablespoon and a half, so don't confuse it with 1/4 cup dry, which is ${g2(gpc / 4)} g and makes about 3/4 cup of porridge.`],
      [`How many rice cups are in 1 kg of steel-cut oats?`, `About ${cups2(1000 / riceCup)} rice-cooker cups. The cup supplied with a rice cooker is a ${RICE_CUP_ML} mL go, which holds about ${g2(riceCup)} g of steel-cut oats at this page's ${g2(gpc)} g per US cup, so 1,000 ÷ ${g2(riceCup)} ≈ ${cups2(1000 / riceCup)}. With Bob's Red Mill's heavier ${STEEL_CUT_BOBS_CUP} g cup it is ${cups2(1000 / (STEEL_CUT_BOBS_CUP / 236.588 * RICE_CUP_ML))} rice cups, and with King Arthur's ${STEEL_CUT_KA_CUP} g it is ${cups2(1000 / (STEEL_CUT_KA_CUP / 236.588 * RICE_CUP_ML))}. In US cups a kilogram is ${cups2(1000 / gpc)} cups, and in 250 mL metric cups ${cups2(1000 / (gpc / 236.588 * 250))}.`],
      [`How do I convert millilitres of steel-cut oats to grams?`, `Multiply by about ${(gpc / 236.588).toFixed(2)}: a US cup of 236.6 mL holds ${g2(gpc)} g, so steel-cut oats run ${(gpc / 236.588).toFixed(2)} g/mL — 100 mL is ${Math.round(gpc / 2.36588)} g, 250 mL is ${Math.round(gpc / 236.588 * 250)} g and 500 mL is ${Math.round(gpc / 236.588 * 500)} g. The other published cup weights give ${(STEEL_CUT_KA_CUP / 236.588).toFixed(2)} g/mL (King Arthur, ${STEEL_CUT_KA_CUP} g) to ${(STEEL_CUT_BOBS_CUP / 236.588).toFixed(2)} g/mL (Bob's Red Mill, ${STEEL_CUT_BOBS_CUP} g). Rolled oats are far lighter at ${(rolled / 236.588).toFixed(2)} g/mL, so a millilitre figure meant for one is wrong for the other by almost half.`],
      [`Is a cup of steel-cut oats 140, 160 or 176 grams?`, `All three are printed by reputable sources, and they are all cut-size and rounding, not errors. Quaker's label (1/4 cup = 40 g) and most store brands give ${g2(gpc)} g per cup — the figure this page uses; King Arthur's chart lists 1/2 cup = 70 g (${STEEL_CUT_KA_CUP} g); Bob's Red Mill prints 1/4 cup = 44 g (${STEEL_CUT_BOBS_CUP} g); Arrowhead Mills gives 1/3 cup = 45 g (135 g). USDA's Foundation Foods entry for steel-cut oats lists no cup measure, so there is no laboratory tie-breaker. Following a brand's own recipe, use its label figure; for anything else, weigh — the ${STEEL_CUT_BOBS_CUP - 135} g spread across a cup is a fifth of the oats.`],
      [`How much cooked oatmeal does 1 cup of steel-cut oats make?`, `About 3 cups. Bob's Red Mill's directions are 1 cup of steel-cut oats to 3 cups of water and 1/2 tsp salt, 10–20 minutes, "yields about 3 cups" for 4 servings. By weight a cup of dry steel-cut oats (${g2(gpc)} g) becomes roughly ${g2(gpc * cookedFactor)} g of porridge — a gain of about ${cookedFactor.toFixed(1)}×, less than rolled oats' ${OATS_COOKED_FACTOR}× because steel-cut is cooked with less water per gram. Quaker's traditional method (1/4 cup oats to 1 1/2 cups water, simmered uncovered 25–30 minutes) boils much of its water off and states no yield. The <a href="/dry-to-cooked/">dry-to-cooked converter</a> covers the other grains.`],
    );
  }
  ...
  if (ing.slug === "quinoa") {
    const ka = QUINOA_KA_CUP;
    const ck = QUINOA_COOKED_CUP;
    const f = QUINOA_COOKED_FACTOR;
    const mlG = (ml) => g2(gpc / 236.588 * ml);
    const riceQuinoa = g2(ingBySlug("white-rice").gramsPerCup + gpc / 2);
    faq.push(
      [`How many grams are in half a cup of uncooked quinoa?`, `About ${g2(gpc / 2)} grams — half of USDA's ${g2(gpc)} g cup. King Arthur's chart, which weighs a cup of whole quinoa at ${ka} g, makes it ${g2(ka / 2)} g, and US brand labels (1/4 cup = 45–47 g) put half a cup at 90–94 g. Anything from about 84 to 95 g is a correctly measured half cup of dry quinoa; the spread is how firmly the cup was filled and how big the seeds are, not an error. Cooked, half a cup is about ${g2(ck / 2)} g.`],
      [`Is 1 cup of quinoa 170 grams or 177 grams?`, `Both are real measurements of a level US cup. USDA's FoodData Central entry for uncooked quinoa measures ${g2(gpc)} g (the figure this page uses), while King Arthur Baking's ingredient chart lists ${ka} g. That is a 4% gap — smaller than the difference between two brands, since Bob's Red Mill's label works out to ${47 * 4} g a cup and Ancient Harvest's to ${45 * 4} g. Use ${g2(gpc)} g for a recipe written against a nutrition database and ${ka} g for one from a US baking site; for a pot of quinoa, either is fine, because the 2:1 water ratio absorbs the difference.`],
      [`What weight is one third of a cup of quinoa?`, `About ${g2(gpc / 3)} grams of dry quinoa (${g2(ka / 3)} g by King Arthur's chart). A third of a cup is a common single main-course portion — it cooks to roughly ${Math.round(gpc / 3 * f)} g, or about 1 cup, of cooked quinoa. If you are measuring quinoa flakes rather than seeds, a third of a cup is 40 g (Ancient Harvest's label serving).`],
      [`How many grams is 1 3/4 cups of quinoa?`, `About ${g2(gpc * 1.75)} grams of dry quinoa — one cup (${g2(gpc)} g) plus three-quarters (${g2(gpc * 0.75)} g). Cook it with 3 1/2 cups of water and it becomes roughly ${Math.round(gpc * 1.75 * f)} g of cooked quinoa, about ${Math.round(gpc * 1.75 * f / ck)} cups, which is 5–6 side servings.`],
      [`How many grams of quinoa is a 160 mL cup?`, `About ${mlG(160)} grams. Dry quinoa is ${g2(gpc)} g per 236.6 mL US cup, which is ${g2(gpc / 236.588 * 100)} g per 100 mL, so a 160 mL cup holds ${mlG(160)} g, a 180 mL rice-cooker cup ${mlG(180)} g, a 200 mL cup ${mlG(200)} g and a 250 mL metric cup ${mlG(250)} g. Cook a 160 mL cup of quinoa with 320 mL of water and you get about ${Math.round(gpc / 236.588 * 160 * f)} g cooked.`],
      [`How many grams of quinoa is half a glass?`, `It depends on the glass, so measure its capacity once with water. A standard 200 mL drinking glass holds ${mlG(200)} g of dry quinoa, so half a glass is ${mlG(100)} g; a 250 mL glass holds ${mlG(250)} g, so half is ${mlG(125)} g. Compare a half US cup at ${g2(gpc / 2)} g. Any of these is a good single serving once cooked — 70–90 g of dry quinoa makes ${Math.round(70 * f)}–${Math.round(90 * f)} g on the plate.`],
      [`How many grams is 1/4 cup of quinoa on the nutrition label?`, `Labels print 42–47 g for a quarter cup of dry quinoa: Bob's Red Mill 47 g, Ancient Harvest 45 g, with Trader Joe's and Kirkland packs listed at 42–44 g. This page's USDA figure gives ${g2(gpc / 4)} g. They all describe the same quarter cup — labels are allowed to round, and a dipped cup packs a little tighter than a spooned one — so for weighing, use ${g2(gpc / 4)} g and for calorie counting use the number on your own bag.`],
      [`Do red, black and tri-color quinoa weigh the same as white?`, `Yes, to within a label rounding. Ancient Harvest prints the same 45 g quarter cup for its red quinoa as for its white, and Bob's Red Mill prints 47 g for both its tri-color and its white, so all of them sit at ${g2(gpc)}–${47 * 4} g per cup. Red and black seeds are the same size as white ones; they differ in a slightly firmer bite and a longer cook time, not in weight, so use this page's chart for any color.`],
      [`How many grams is a cup of cooked quinoa?`, `About ${ck} grams (USDA measures a cooked cup at ${ck} g; King Arthur's chart says 184 g), so half a cup of cooked quinoa is about ${g2(ck / 2)} g and a quarter cup ${g2(ck / 4)} g. That cooked cup started as roughly ${Math.round(ck / f)} g of dry quinoa, a little more than a third of a cup. Cooked quinoa weighs more per cup than cooked white rice (158 g) because it holds more water — about 72% of its cooked weight.`],
      [`How much cooked quinoa does 1 cup of dry quinoa make?`, `About ${Math.round(gpc * f)} grams of cooked quinoa, which is roughly 3 cups. The Whole Grains Council's chart is 1 cup quinoa plus 2 cups liquid makes 3 cups cooked in 12–15 minutes; Bob's Red Mill quotes the same 2:1 ratio and gets 3 1/2–4 fluffier cups in an Instant Pot. By weight the factor is about ${f}× (USDA), so the "quinoa quadruples" claim is a myth. The dry-to-cooked converter runs this both ways for rice, oats and pasta too.`],
      [`Is there 48 grams of whole grains in half a cup of quinoa?`, `Not as a weight in the cup. Half a US cup of dry quinoa is ${g2(gpc / 2)} g on the scale, and half a cup of cooked quinoa about ${g2(ck / 2)} g. The 48 comes from labelling: the Whole Grains Council defines one whole-grain serving as 16 g of whole-grain ingredient and recommends at least three a day, so 48 g is a full day's target. Because dry quinoa is 100% whole grain, ${g2(gpc / 2)} g of it is more than five such servings, and half a cup cooked (about ${Math.round(ck / 2 / f)} g of grain before the water) is nearly two.`],
      [`How many calories are in a cup of quinoa?`, `About ${Math.round(gpc * QUINOA_DRY_100.kcal / 100)} kcal in a cup of dry quinoa (USDA: ${QUINOA_DRY_100.kcal} kcal per 100 g), with ${g2(gpc * QUINOA_DRY_100.protein / 100)} g of protein and ${g2(gpc * QUINOA_DRY_100.fiber / 100)} g of fiber — that dry cup cooks into about 3 cups, so a cup of cooked quinoa is ${Math.round(ck * QUINOA_COOKED_100.kcal / 100)} kcal, ${g2(ck * QUINOA_COOKED_100.protein / 100)} g protein and ${g2(ck * QUINOA_COOKED_100.fiber / 100)} g fiber. Half a cup dry is ${Math.round(gpc / 2 * QUINOA_DRY_100.kcal / 100)} kcal; half a cup cooked ${Math.round(ck / 2 * QUINOA_COOKED_100.kcal / 100)} kcal. Brand labels agree with the dry figure (Bob's Red Mill and Ancient Harvest both print 170 kcal per quarter cup).`],
      [`Should I weigh quinoa before or after rinsing?`, `Before. Rinsed quinoa carries surface water and starts absorbing more within minutes, so a drained cup no longer weighs ${g2(gpc)} g and its water ratio is thrown off. Measure or weigh dry, then rinse in a fine sieve if the bag says to. Bob's Red Mill and Ancient Harvest both state their quinoa is pre-rinsed to remove the bitter saponin coating, though the Whole Grains Council still suggests a quick rinse for any residue — it only affects taste, not weight, as long as you weigh first.`],
      [`How many grams is a cup of quinoa flour or quinoa flakes?`, `Quinoa flour is about 110 g per cup on King Arthur's chart, and Bob's Red Mill's label (1/4 cup = 28 g) works out to 112 g — lighter than all-purpose flour's ${g2(ingBySlug("all-purpose-flour").gramsPerCup)} g, so swap it by weight. Quinoa flakes are lighter still: Ancient Harvest's label gives 1/3 cup = 40 g, or ${40 * 3} g per cup, much like rolled oats (${g2(ingBySlug("rolled-oats").gramsPerCup)} g). Neither is in this page's calculator, which is for whole dry seeds only.`],
      [`How much do 1 cup of rice and 1/2 cup of quinoa weigh together?`, `${riceQuinoa} g dry — ${g2(ingBySlug("white-rice").gramsPerCup)} g of long-grain white rice plus ${g2(gpc / 2)} g of quinoa. Cooked together with about 3 cups of water it makes roughly ${Math.round(ingBySlug("white-rice").gramsPerCup * 2.8 + gpc / 2 * f)} g, enough for 4–6 servings. The recipe weight calculator adds any list of ingredients like this in one go.`],
    );
  }
  ...
  if (ing.slug === "oat-flour") {
    const usda = OAT_FLOUR_USDA_CUP;
    const apg = ingBySlug("all-purpose-flour").gramsPerCup;
    const oats = ingBySlug("rolled-oats").gramsPerCup;
    const mlG = (ml) => g2(gpc / 236.588 * ml);
    const kcalCup = Math.round(gpc * OAT_FLOUR_100.kcal / 100);
    faq.push(
      [`Is 1 cup of oat flour 92 grams or 104 grams?`, `Both are real measurements of a level US cup. King Arthur Baking's ingredient chart lists oat flour at 3 1/4 oz, or ${g2(gpc)} g — the figure this page and its calculator use — while USDA's FoodData Central entry for oat flour (partially debranned, #169741) measures ${usda} g. The 13% gap is mostly how the cup was filled: King Arthur spoons flour lightly into the cup and levels it; a cup dipped into the bag packs oat flour down, because it is fluffy and compacts easily. Use ${g2(gpc)} g for a recipe from a US baking site written in cups, and ${usda} g when working from a nutrition database.`],
      [`Why do I get 138 g, 156 g and 180 g for 1 1/2 cups of oat flour?`, `Because three different cups are being converted. By King Arthur's chart (this page), 1 1/2 cups of oat flour is ${g2(gpc * 1.5)} g; by USDA's ${usda} g cup it is ${g2(usda * 1.5)} g; and by the Bob's Red Mill or Quaker label serving of 1/4 cup = 30 g it is ${30 * 6} g. King Arthur's own bag label (1/3 cup = 28 g) gives just ${28 * 4.5} g. All describe a level US cup — labels are allowed to round, and dipped cups pack tighter than spooned ones. If the recipe was written in cups, ${g2(gpc * 1.5)} g is the safest reading; if it came with a gram weight, use the grams and ignore the cups.`],
      [`How many mL is 1/4 cup of oat flour, and how many grams is that?`, `A quarter US cup is ${g2(59.147)} mL of volume, and that much oat flour weighs about ${mlG(59.147)} g (${g2(usda / 4)} g by USDA's heavier cup). Millilitres measure space, not weight, so 59 mL of oat flour is 59 mL whatever it weighs; the grams are what change from ingredient to ingredient. The same 59 mL holds ${g2(apg / 4)} g of all-purpose flour. Oat flour is about ${mlG(100)} g per 100 mL, so a 250 mL metric cup holds ${mlG(250)} g and a 200 mL glass ${mlG(200)} g.`],
      [`How many grams of oat flour is 100 mL, or a 250 mL metric cup?`, `100 mL of oat flour is about ${mlG(100)} g, and a level 250 mL metric cup — the standard in the UK, Australia and New Zealand — about ${mlG(250)} g (${g2(usda / 236.588 * 250)} g by USDA's figure). A 236.6 mL US cup is ${g2(gpc)} g, so a metric cup holds about 6% more; if a recipe from an Australian site says "1 cup oat flour", weigh ${mlG(250)} g rather than ${g2(gpc)} g. Half a metric cup is ${mlG(125)} g.`],
      [`How much oat flour replaces 1/2 cup of all-purpose flour?`, `Weigh ${g2(apg / 2)} g of oat flour — the same weight as the half cup of all-purpose flour it replaces (${g2(apg / 2)} g), which is about ${cups2(apg / 2 / gpc)} cups, a scant 2/3 cup. A plain half cup of oat flour is only ${g2(gpc / 2)} g, ${g2(apg / 2 - gpc / 2)} g short, because oat flour is about ${Math.round((1 - gpc / apg) * 100)}% lighter than wheat flour per cup. Quaker's guidance says the same thing by volume: 1 1/4 cups of oat flour for every cup of all-purpose, so 5/8 cup (10 tablespoons) for half a cup. Arrowhead Mills likewise says to swap by equal weight, not volume.`],
      [`Can I substitute oat flour for all-purpose flour 1:1?`, `By weight, yes: Quaker's Oat Flour 101 guide states oat flour "can be substituted for all-purpose wheat flour at a 1:1 ratio by weight", and Arrowhead Mills says to use it "by equal weight instead of volume". By the cup, no — a cup of oat flour is ${g2(gpc)} g against ${g2(apg)} g, so you need about 1 1/4 to 1 1/3 cups of oat flour per cup of all-purpose. The bigger limit is gluten: oat flour has none, so anything that has to rise needs wheat flour for structure. Quaker suggests swapping only about a third of the flour in bread; King Arthur says oat flour works for a portion of the flour in tender bakes like muffins, cakes and cookies, and its retail bag says up to half in non-yeasted recipes; Bob's Red Mill recommends adding xanthan gum when going fully gluten-free.`],
      [`How much oat flour does 1 cup of rolled oats make?`, `About ${g2(oats)} g of oat flour — grinding changes the volume, not the weight, so ${g2(oats)} g of rolled oats (1 cup) becomes ${g2(oats)} g of flour, which is very nearly 1 cup (${g2(gpc)} g) of spooned oat flour by weight. None of the big mills publish a cup-for-cup yield, because freshly ground oats settle unevenly, so the reliable method is to weigh the oats before blending: for a recipe that wants 1 cup of oat flour, grind ${g2(gpc)} g of rolled oats; for 1 1/2 cups, grind ${g2(gpc * 1.5)} g. King Arthur's recipes do exactly this — "1 1/2 cups (134 g) old-fashioned rolled oats, ground for 30 seconds in a blender".`],
      [`How many grams is 1/4 cup of oat flour on the nutrition label?`, `Bob's Red Mill and Quaker both print 1/4 cup = 30 g, which is ${30 * 4} g per cup; King Arthur's bag says 1/3 cup = 28 g (${28 * 3} g per cup) and Arrowhead Mills and Gold Medal 1/3 cup = 30 g (${30 * 3} g per cup). This page's chart gives ${g2(gpc / 4)} g for a quarter cup. The labels are not wrong — serving sizes are rounded to a convenient fraction and the nearest gram, and a dipped cup packs more than a spooned one — but they show why "1/4 cup" can mean anything from 21 to 30 g. For weighing, use ${g2(gpc / 4)} g; for counting calories, use the grams printed on your own bag.`],
      [`How many calories are in a cup of oat flour?`, `About ${kcalCup} kcal in a level cup (${g2(gpc)} g), using USDA's ${OAT_FLOUR_100.kcal} kcal per 100 g for oat flour. That cup also carries about ${g2(gpc * OAT_FLOUR_100.protein / 100)} g of protein, ${g2(gpc * OAT_FLOUR_100.fat / 100)} g of fat and ${g2(gpc * OAT_FLOUR_100.fiber / 100)} g of fiber. Half a cup is ${Math.round(kcalCup / 2)} kcal and a quarter cup ${Math.round(kcalCup / 4)} kcal. King Arthur's label agrees closely — 110 kcal per 28 g serving, about 393 per 100 g. A cup measured by USDA's heavier ${usda} g fill would be ${Math.round(usda * OAT_FLOUR_100.kcal / 100)} kcal, which is one more reason to weigh rather than scoop when the numbers matter.`],
      [`Is oat flour gluten-free?`, `Oats contain no gluten, but ordinary oat flour is often not gluten-free in practice: the Celiac Disease Foundation notes that oats are commonly grown, processed and transported alongside wheat, barley and rye, and advises eating only oat products with a gluten-free label. Quaker's own guide states its oat flour is not certified gluten-free for that reason, while Bob's Red Mill mills its oat flour in a dedicated gluten-free facility and King Arthur's is certified by the GFCO. Certification changes nothing about the weight: certified and uncertified oat flour both weigh ${g2(gpc)} g per spooned cup.`],
      [`Does oat flour weigh the same as rolled oats per cup?`, `Nearly. A cup of rolled oats is about ${g2(oats)} g and a cup of oat flour ${g2(gpc)} g on King Arthur's chart (89 g and 92 g on the chart itself; this site uses 90 g for oats) — grinding lets the flour fill the gaps between flakes, but the powder is so fluffy that it barely gains. Steel-cut oats are the outlier at about ${g2(ingBySlug("steel-cut-oats").gramsPerCup)} g per cup, and oat bran sits near ${94} g. So a recipe that swaps rolled oats for oat flour cup-for-cup is roughly right by weight, which is not true of the swap with wheat flour (${g2(apg)} g).`],
      [`How many cups of oat flour are in a 500 g or 1 kg bag?`, `About ${cups2(500 / gpc)} cups in 500 g and ${cups2(1000 / gpc)} cups in a kilogram, spooned and levelled at ${g2(gpc)} g a cup — or ${cups2(500 / usda)} and ${cups2(1000 / usda)} cups by USDA's ${usda} g fill. A 1 lb (${Math.round(16 * OZ)} g) bag holds about ${cups2(16 * OZ / gpc)} cups, and King Arthur's 48 oz bag (${Math.round(48 * OZ)} g) about ${cups2(48 * OZ / gpc)} cups. Working backwards from a recipe, 100 g of oat flour is ${cups2(100 / gpc)} cups and 200 g is ${cups2(200 / gpc)} cups.`]
    );
  }
  if (ing.slug === "grated-parmesan") {
    const rasp = parmGrateCup(0.5), med = parmGrateCup(1 / 3), coarse = parmGrateCup(0.25);
    const shaker = PARM_KRAFT_SHAKER_CUP;
    const cheddar = ingBySlug("shredded-cheddar").gramsPerCup;
    const kcalCup = Math.round(gpc * PARM_100.kcal / 100);
    faq.push(
      [`How many grams is 1 cup of grated parmesan?`, `${g2(gpc)} grams for a level US cup of finely grated parmesan — the figure on King Arthur Baking's ingredient chart (1/2 cup = 1 3/4 oz = 50 g) and USDA FoodData Central's measured cup (#171247). That is the canister-style fine grate. Grate a wedge yourself and the same cup holds less: about ${Math.round(rasp)} g from a Microplane or ${Math.round(med)} g from the medium holes of a box grater, by America's Test Kitchen's yields. When a recipe gives a weight, use the weight.`],
      [`How many grams is 1/2 cup of grated parmesan?`, `${g2(gpc / 2)} grams — this is the exact row on King Arthur's chart, 1/2 cup = 1 3/4 oz = 50 g. Half a cup from the Kraft shaker (2 tsp = 5 g on the label) works out to ${g2(shaker / 2)} g, and half a cup of fluffy Microplane-grated parmesan to only about ${Math.round(rasp / 2)} g, so the "right" answer depends on how fine the cheese is. On a scale, 50 g is 1/2 cup of the fine grade most recipes assume.`],
      [`How many grams is 1/4 cup of grated parmesan (in "EU grams")?`, `${g2(gpc / 4)} grams — a quarter of the ${g2(gpc)} g cup. Grams are grams on both sides of the Atlantic; what changes is the cup: a US cup is 236.6 mL and a 250 mL metric cup (UK, Australia, NZ) holds ${g2(gpc / 236.588 * 250 / 4)} g in its quarter. If you are reading a US label instead, Kraft's finely shredded and Sargento's fine-cut parmesan both print 1/4 cup = ${PARM_LABEL_QUARTER_CUP} g, and 4 tablespoons at the label's 5 g each is 20 g.`],
      [`How many grams is 1/3 cup of parmesan?`, `About ${g2(gpc / 3)} grams grated fine (${g2(gpc)} g per cup ÷ 3). From the Kraft shaker it is ${g2(shaker / 3)} g, and a third of a cup of Microplaned parmesan about ${Math.round(rasp / 3)} g. Notice that America's Test Kitchen's rule makes 1/3 cup of medium-grated cheese exactly 1 oz (${g2(OZ)} g) — a handy round number if you grate from a wedge on a box grater.`],
      [`How many grams is 1 1/4 cups of grated parmesan cheese?`, `${g2(gpc * 1.25)} grams at ${g2(gpc)} g per cup. The same 1 1/4 cups is ${g2(shaker * 1.25)} g from the Kraft shaker, ${g2(PARM_LABEL_QUARTER_CUP * 5)} g by the 1/4 cup = 28 g shredded-parmesan labels, or roughly ${Math.round(rasp * 1.25)} g if you Microplane a wedge. For 1 1/2 cups use ${g2(gpc * 1.5)} g, for 2 cups ${g2(gpc * 2)} g — the converter above takes any amount.`],
      [`How many grams is a tablespoon of grated parmesan?`, `Call it 5 to ${g2(gpc / 16)} grams. Every US label — Kraft, BelGioioso, 4C — prints 1 tablespoon = 5 g (Kraft's shaker says 2 teaspoons = 5 g, which is 7.5 g per tablespoon), and USDA lists the same 5 g spoon. Divide this page's ${g2(gpc)} g cup by 16 and you get ${g2(gpc / 16)} g. USDA's own record carries both numbers, and 16 × 5 g is 80 g, not 100 g: a tablespoon is filled loosely and a cup settles as it fills. For a sprinkle it does not matter; for a sauce or a dough, weigh it.`],
      [`Why does the Kraft shaker say 2 teaspoons = 5 grams?`, `Because the canister's fine, dry, evenly milled parmesan packs denser than a home-grated cup: 2 tsp = 5 g is ${g2(5 / 2 * 3)} g per tablespoon and ${shaker} g per cup, about 20% above the ${g2(gpc)} g of King Arthur and USDA. So a cup scooped from the green can is ${shaker} g and half a cup ${g2(shaker / 2)} g. If a recipe was written with shaker cheese in mind, it also has far less flavour per gram than a Parmigiano wedge, so weigh generously rather than skimp.`],
      [`How much does 1 cup of shaved parmesan weigh?`, `Neither USDA nor King Arthur publishes a cup weight for shaved parmesan, so treat any figure as an estimate. Shavings are the loosest-packed form of all — broad curls that trap large air gaps — so a cup will land at or below the fluffy Microplane figure of about ${Math.round(rasp)} g, often nearer 40–50 g when the curls are large. Shaved cheese is almost always a garnish, so the practical answer is "a good handful"; for anything cooked, weigh the cheese before shaving.`],
      [`How many cups of grated parmesan does a 4 oz or 8 oz wedge make?`, `By America's Test Kitchen's yields, 1 oz of hard cheese gives 1/2 cup on a Microplane, 1/3 cup on a medium box grater or 1/4 cup on the coarse side. So a 4 oz wedge (${g2(4 * OZ)} g) makes about 2 cups Microplaned, 1 1/3 cups medium-grated or 1 cup coarse; an 8 oz wedge doubles that. At this page's fine ${g2(gpc)} g cup a 4 oz wedge is ${cups2(4 * OZ / gpc)} cups, and a 100 g piece is exactly 1 cup. The weight never changes when you grate — only the volume does.`],
      [`Does freshly grated parmesan weigh the same as pre-grated?`, `Not per cup. The pre-grated canister products are milled fine and dry, so they pack: Kraft's shaker label implies ${shaker} g per cup, the refrigerated shredded tubs (Kraft, Sargento, 1/4 cup = 28 g) 112 g, and the King Arthur / USDA fine grate ${g2(gpc)} g. A wedge grated at home on a Microplane weighs about ${Math.round(rasp)} g per cup, or ${Math.round(med)} g on a medium grater — a fresh cup can hold half the cheese of a canister cup. Recipes that say "1 cup grated" almost always mean the fine, 100 g cup; if they say "finely grated on a rasp", weigh out ${Math.round(rasp)} g per cup.`],
      [`How many calories are in a cup of grated parmesan?`, `About ${kcalCup} kcal in a level ${g2(gpc)} g cup, using USDA's ${PARM_100.kcal} kcal per 100 g for grated parmesan (#171247), with ${g2(gpc * PARM_100.protein / 100)} g protein and ${g2(gpc * PARM_100.fat / 100)} g fat. A label tablespoon (5 g) is about ${Math.round(5 * PARM_100.kcal / 100)} kcal, which Kraft, BelGioioso and 4C round to 20. Parmesan is also very salty: USDA lists ${PARM_100.sodium} mg sodium per 100 g, so a cup carries about ${(gpc * PARM_100.sodium / 100 / 1000).toFixed(1)} g of sodium and ${Math.round(gpc * PARM_100.calcium / 100)} mg of calcium.`],
      [`Is Parmigiano Reggiano the same as parmesan for measuring?`, `For the scale, yes: any hard, dry grating cheese — Parmigiano Reggiano, Grana Padano, domestic "parmesan", Pecorino Romano — weighs about the same per cup at the same grate size, so every figure on this page applies. The difference is legal and culinary. Parmigiano Reggiano is a PDO product made only in Parma, Reggio Emilia, Modena and parts of Bologna and Mantua, aged at least 12 months (most of what is sold is 24–36 months), and even the grated version must be grated inside the region with 25–35% moisture and no more than 18% rind. Because it is drier and more crystalline than young domestic parmesan it tends to grate finer, which is one more reason to weigh.`],
      [`How many cups is 100 g or 200 g of grated parmesan?`, `100 g is exactly 1 cup finely grated on this page's figure, and 200 g is 2 cups. Grated on a Microplane the same 100 g fills about ${cups2(100 / rasp)} cups, and from the Kraft shaker 100 g is only ${cups2(100 / shaker)} cup. A typical 120 g UK tub of grated Parmigiano Reggiano is therefore about 1 1/4 cups fine, and a 4 oz (${g2(4 * OZ)} g) US shaker just under 1 cup by its own 5 g-per-2-tsp density (${cups2(4 * OZ / shaker)} cups).`],
      [`Does grated parmesan weigh the same as shredded cheddar or mozzarella?`, `Close, and lighter: ${g2(gpc)} g against ${g2(cheddar)} g for a cup of shredded cheddar, jack, mozzarella or Swiss (the 4 oz-per-cup rule). Softer cheeses shred into thicker strands that pack heavier, while parmesan is drier and grated finer, so the same cup holds about ${Math.round((1 - gpc / cheddar) * 100)}% less. For the whole family — crumbled feta at 150 g, cream cheese and ricotta near 227–232 g — see the cheese equivalents section of the dairy conversion chart, linked below.`],
    );
  }
...
<h2>Oats by the rice cup, 100 mL and the metric cup</h2>
<p>Three of the most common ways to measure oats without a scale are not the US cup at all. The <strong>rice-cooker cup</strong> that ships with every Zojirushi, Aroma or Panasonic is a Japanese <em>go</em> of ${RICE_CUP_ML} mL — three-quarters of a US cup — and holds about <strong>${g2(gpc / 236.588 * RICE_CUP_ML)} g of rolled oats</strong> (${g2(OATS_USDA_CUP / 236.588 * RICE_CUP_ML)} g by USDA's lighter cup, and about ${g2(ingBySlug("steel-cut-oats").gramsPerCup / 236.588 * RICE_CUP_ML)} g of <a href="/cups-to-grams/steel-cut-oats/">steel-cut oats</a>). The <strong>100 mL</strong> line on a measuring jug is ${Math.round(gpc / 2.36588)} g. The <strong>250 mL metric cup</strong> of the UK, Australia and New Zealand is ${Math.round(gpc / 236.588 * 250)} g. All of it comes off one number: rolled oats sit at about <strong>${(gpc / 236.588).toFixed(2)} g/mL</strong> on this page — USDA's cup gives ${(OATS_USDA_CUP / 236.588).toFixed(2)}, Bob's Red Mill's label ${(OATS_LABELS.bobsHalfCup * 2 / 236.588).toFixed(2)} — so multiply any millilitre figure by 0.34–0.41 and you have the honest range. Steel-cut oats are almost twice as dense (${(ingBySlug("steel-cut-oats").gramsPerCup / 236.588).toFixed(2)} g/mL), which is why the table carries them as a separate column.</p>
${oatsMlTable(gpc, ingBySlug("steel-cut-oats").gramsPerCup)}
<p class="note">Computed from ${g2(gpc)} g (this page), ${OATS_USDA_CUP} g (USDA #173904) and ${g2(ingBySlug("steel-cut-oats").gramsPerCup)} g of steel-cut per 236.588 mL. Measuring the water with the same rice cup? A rice cup of <a href="/cups-to-grams/white-rice/">white rice</a> weighs ${g2(ingBySlug("white-rice").gramsPerCup / 236.588 * RICE_CUP_ML)} g — twice the oats — so the cooker's own water lines are no guide for porridge; use two rice cups of water per rice cup of oats (Quaker's 1 : 2 by volume). Any other cup, glass or spoon size: the <a href="/volume-converter/">volume converter</a>.</p>
<h2>How many whole-grain serves are in a cup of oats?</h2>
<p>Oats are a 100% whole grain — the flake is the entire groat, bran and germ included, rolled flat — so <strong>the grams of oats on the scale are the grams of whole grain</strong>, and every serving system starts from that weight. There are three systems in everyday use. The <strong>Whole Grains Council</strong> counts 16 g of whole grain as one serving (its 100% Stamp requires 16 g per labelled serving, the basic Stamp 8 g). <strong>USDA's MyPlate</strong> counts grains in ounce-equivalents: for oatmeal, 1 oz-equivalent is 1/2 cup cooked, one instant packet, or 1 oz (about 1/3 cup) of dry oats. And the <strong>Australian Dietary Guidelines</strong> — whose serve sizes New Zealand's Ministry of Health has adopted — define a standard serve of grain food as roughly 500 kJ: 1/2 cup (120 g) of cooked porridge, or 1/4 cup (30 g) of muesli.</p>
<p>The Australian serve is defined on the cooked bowl, so it has to be converted back to dry oats. On the energy basis behind the guideline, USDA's ${(OATS_100.kcal * 4.184 / 100).toFixed(1)} kJ per gram of dry oats puts a 500 kJ serve at about ${OATS_SERVE_AU_G} g — the same ${OATS_SERVE_AU_G} g the guideline itself uses for muesli — so the table divides by ${OATS_SERVE_AU_G}. Converting by cooked weight instead (120 g of porridge is about ${g2(120 / OATS_COOKED_FACTOR)} g of dry oats at Quaker's 1/2 cup + 1 cup water) gives roughly a third more serves; a quarter to a third of a cup of dry oats per serve covers both readings.</p>
${oatsServesTable(gpc)}
<p>So <strong>1 cup and 1/3 of oats — ${g2(gpc * 4 / 3)} g dry — is about ${Math.round(gpc * 4 / 3 / OATS_SERVE_AU_G)} standard serves</strong> (Australia and NZ), ${(gpc * 4 / 3 / OZ).toFixed(1)} ounce-equivalents (USDA) or ${(gpc * 4 / 3 / OATS_SERVE_WGC_G).toFixed(1)} Whole Grains Council servings; and <strong>half a cup of uncooked oats carries ${g2(gpc / 2)} g of whole grain</strong> — labels print 40 g because Quaker's half-cup serving weighs 40 g. Going the other way, a cup of cooked oatmeal (${OATS_COOKED_CUP} g, USDA) comes from about ${g2(OATS_COOKED_CUP / OATS_COOKED_FACTOR)} g of dry oats — a rounded half cup, because oats gain ${OATS_COOKED_FACTOR}× their weight in the pot — which makes it 2 US ounce-equivalents by the half-cup-cooked rule and just under 2 Australian serves by the 120 g rule. On the nutrition side USDA lists dry oats at ${OATS_100.kcal} kcal per 100 g with ${OATS_100.protein} g protein, ${OATS_100.fat} g fat and ${OATS_100.fiber} g fiber, so a level cup (${g2(gpc)} g) is about ${Math.round(gpc * OATS_100.kcal / 100)} kcal and a half cup ${Math.round(gpc / 2 * OATS_100.kcal / 100)} kcal; Quaker's 150 kcal per 40 g agrees. The <a href="/dry-to-cooked/">dry-to-cooked converter</a> does the porridge math for any amount, and the <a href="/portion-calculator/">portion calculator</a> scales a breakfast for a crowd.</p>` : ""}${ing.slug === "steel-cut-oats" ? `
<h2>Is a cup of steel-cut oats 140, 160 or 176 g?</h2>
<p>Steel-cut oats are chopped groats rather than flattened flakes, so they pour like a grain and pack far heavier than rolled oats — <strong>about ${g2(gpc)} g per US cup here, against ${g2(ingBySlug("rolled-oats").gramsPerCup)} g for <a href="/cups-to-grams/rolled-oats/">rolled oats</a></strong>. But the published figures spread more than for any other oat product. <strong>Quaker's label serving is 1/4 cup = 40 g (${g2(gpc)} g per cup, the figure this page and its calculator use)</strong>, and Whole Foods, Publix, Harris Teeter and Market Basket print the same. <strong>King Arthur's ingredient chart lists 1/2 cup = 70 g (${STEEL_CUT_KA_CUP} g)</strong>. Arrowhead Mills and Rocky Mountain give 1/3 cup = 45 g (135 g). <strong>Bob's Red Mill's label reads 1/4 cup = 44 g (${STEEL_CUT_BOBS_CUP} g)</strong>. USDA's Foundation Foods entry for steel-cut oats (#2346397) lists no cup measure at all, so there is no laboratory tie-breaker. The spread is cut size — a coarser cut leaves more air between the pieces — plus label rounding to the nearest gram on a small serving, and a level cup lands somewhere in the <strong>135–176 g</strong> band.</p>
${steelCutSourcesTable()}
<p>So <strong>a quarter cup of dry steel-cut oats is ${g2(gpc / 4)} g</strong> on this page (35–44 g across the labels), half a cup ${g2(gpc / 2)} g, and 100 g is ${cups2(100 / gpc)} cups — a rounded two-thirds. If you are following a brand's own recipe, use its label figure; for anything else, weigh. Two sanity checks: a number near ${g2(ingBySlug("rolled-oats").gramsPerCup)} g per cup is rolled oats, not steel-cut, and anything over about 180 g was heaped or is a 250 mL metric cup (${Math.round(gpc / 236.588 * 250)}–${Math.round(STEEL_CUT_BOBS_CUP / 236.588 * 250)} g).</p>
<h2>Steel-cut oats by the millilitre, rice cup and kilogram</h2>
<p>Because steel-cut oats pour like rice, people measure them like rice. The <strong>rice-cooker cup</strong> is a Japanese <em>go</em> of ${RICE_CUP_ML} mL, three-quarters of a US cup, and holds about <strong>${g2(gpc / 236.588 * RICE_CUP_ML)} g of steel-cut oats</strong> — so <strong>a 1 kg bag is about ${cups2(1000 / (gpc / 236.588 * RICE_CUP_ML))} rice cups</strong> (${cups2(1000 / gpc)} US cups, or ${cups2(1000 / (gpc / 236.588 * 250))} metric cups). Any millilitre figure converts at about <strong>${(gpc / 236.588).toFixed(2)} g/mL</strong>: 100 mL is ${Math.round(gpc / 2.36588)} g, 250 mL is ${Math.round(gpc / 236.588 * 250)} g. The table shows the same measures at King Arthur's and Bob's Red Mill's cup weights so you can see how much the choice of source moves the answer — about 10% either side.</p>
${steelCutMlTable(gpc)}
<p class="note">Computed from ${g2(gpc)}, ${STEEL_CUT_KA_CUP} and ${STEEL_CUT_BOBS_CUP} g per 236.588 mL. Rolled oats run ${(ingBySlug("rolled-oats").gramsPerCup / 236.588).toFixed(2)} g/mL, so a millilitre figure meant for one kind of oat is wrong for the other by almost half. The <a href="/volume-converter/">volume converter</a> handles any other cup or glass size.</p>
<h2>Cooked steel-cut oats: what a quarter cup weighs</h2>
<p>Cooked steel-cut porridge is mostly water, so a cooked cup weighs about what any cooked oatmeal weighs — <strong>roughly water density, ${OATS_COOKED_CUP}–${STEEL_CUT_COOKED_CUP} g per cup</strong>. USDA's measured cup of oatmeal cooked with water is ${OATS_COOKED_CUP} g (#173905). Bob's Red Mill's steel-cut method — 1 cup of oats (${STEEL_CUT_BOBS_CUP} g by its label) to 3 cups of water, 10–20 minutes, "yields about 3 cups" — starts from ${STEEL_CUT_BOBS_CUP + Math.round(3 * 236.588)} g in the pot and only lands at 3 cups once about 150 g has left as steam, which works out to about ${STEEL_CUT_COOKED_CUP} g per cooked cup. Either way, <strong>1/4 cup of cooked steel-cut oats is about 60 g</strong> (${g2(OATS_COOKED_CUP / 4)}–${g2(STEEL_CUT_COOKED_CUP / 4)} g), and it contains only about ${g2(STEEL_CUT_BOBS_CUP / 12)} g of dry oats — a rounded tablespoon and a half. Don't confuse that with 1/4 cup <em>dry</em>, which is ${g2(gpc / 4)} g and makes about three-quarters of a cup of porridge. Quaker's traditional method uses more water (1/4 cup oats to 1 1/2 cups water, simmered uncovered 25–30 minutes) and boils much of it off; it states no yield, so the 3-cups-per-cup figure above is Bob's. Cooked weight gain is about ${(STEEL_CUT_COOKED_CUP * 3 / STEEL_CUT_BOBS_CUP).toFixed(1)}×, against ${OATS_COOKED_FACTOR}× for rolled oats — the <a href="/dry-to-cooked/">dry-to-cooked converter</a> has every grain's factor.</p>` : ""}${ing.slug === "chocolate-chips" ? `
...
<p>Cooking rather than baking? A <strong>rice-cooker cup is 180 mL</strong> (the Japanese <em>gō</em>), about three-quarters of a US cup — roughly <strong>${Math.round(gpc / 236.588 * 180)} g</strong> of long-grain rice or ${Math.round(200 / 236.588 * 180)} g of short-grain, which is why a "5-cup" cooker holds less than five US cups. A 250 mL metric cup (UK, Australia, New Zealand) holds about ${Math.round(gpc / 236.588 * 250)} g. And weigh rice <em>dry, before rinsing</em>: rinsed grains hold surface and absorbed water (soaking can add around 20% to the weight), so a drained cup is no longer ${g2(gpc)} g. Want the cooked amount instead? A cup of dry rice cooks up to roughly 3 cups — the <a href="/dry-to-cooked/">dry-to-cooked converter</a> does that math both ways, and the <a href="/portion-calculator/">portion calculator</a> turns it into grams of dry rice per person.</p>` : ""}${ing.slug === "quinoa" ? `
<h2>170 g or 177 g? What a cup of dry quinoa weighs, by source</h2>
<p>Look up a cup of quinoa in two reference charts and you will get two numbers. <strong>USDA's FoodData Central measures 1 cup of uncooked quinoa at ${g2(gpc)} g</strong>, the figure this page and its calculator use. <strong>King Arthur Baking's ingredient chart lists whole quinoa at 6 1/4 oz, or ${QUINOA_KA_CUP} g.</strong> The brand labels sit in between and a little above: Ancient Harvest prints 1/4 cup = 45 g (${45 * 4} g a cup) and Bob's Red Mill 1/4 cup = 47 g (${47 * 4} g a cup). So <strong>half a cup of uncooked quinoa is about ${g2(gpc / 2)} g</strong>, a third of a cup ${g2(gpc / 3)} g, and a quarter cup ${g2(gpc / 4)} g — and if another source says 88, 59 or 47 g for the same measures, it is using King Arthur's or a label's cup, not a different quinoa.</p>
${quinoaTypesTable()}
<p class="note">Dry weights unless marked cooked. USDA SR Legacy #168874 (uncooked) and #168917 (cooked); King Arthur Baking ingredient weight chart; brand nutrition labels as printed, multiplied up to a cup (labels round to the gram). Half-cup, quarter-cup and 100 g figures are computed from the cup weight.</p>
<p>Two rows to read carefully. <strong>Red and tri-color quinoa weigh the same as white</strong> — each brand prints the identical serving weight for all its colors, because the seeds are the same size; the darker colors just cook a few minutes longer. And the two processed forms are much lighter: <strong>quinoa flakes are about ${40 * 3} g a cup</strong> (steamed and rolled flat like oats) and <strong>quinoa flour about 110 g</strong>, so a recipe calling for a cup of either is asking for well under half the weight of a cup of seeds. This page's calculator is for whole dry seeds only.</p>
<p>A quick sanity check for any number you meet: a level US cup of dry quinoa lands between about <strong>165 and 190 g</strong>, depending on the source and how firmly the cup was filled. A figure near 185 g might be a cooked cup rather than a heavily packed dry one; 120 g or less is flakes or flour; and anything above 190 g is suspect for a level US cup — a 250 mL metric cup holds ${Math.round(gpc / 236.588 * 250)}–${Math.round(QUINOA_KA_CUP / 236.588 * 250)} g, and beyond that the cup was heaped.</p>
<h2>Quinoa in a 160 mL, 180 mL, 200 mL or 250 mL cup</h2>
<p>Not every "cup" is the 236.6 mL US cup this chart assumes. Rice cookers come with a 180 mL cup, metric recipes in the UK, Australia and New Zealand mean 250 mL, and plenty of people measure grain with the glass or coffee cup they have to hand. Quinoa is ${g2(gpc / 236.588 * 100)} g per 100 mL, so the arithmetic is easy — here it is done for the common sizes, with the cooked weight each one becomes at the standard 2 parts water to 1 part quinoa:</p>
${quinoaCupSizesTable(gpc)}
<p class="note">Dry weights computed from ${g2(gpc)} g per 236.588 mL; cooked weights at the USDA-derived ×${QUINOA_COOKED_FACTOR} factor. Half a 200 mL glass of quinoa is ${g2(gpc / 236.588 * 100)} g; half a 250 mL glass ${g2(gpc / 236.588 * 125)} g. If your glass is a different size, fill it with water once and weigh it: the millilitres equal the grams of water.</p>
<h2>Dry to cooked: what a cup of quinoa becomes</h2>
<p>Quinoa cooks at <strong>2 parts liquid to 1 part quinoa</strong> — the Whole Grains Council, Bob's Red Mill and Ancient Harvest all give the same ratio — and <strong>1 cup of dry quinoa yields about 3 cups cooked</strong> in 12–15 minutes on the hob (Bob's Red Mill gets a fluffier 3 1/2–4 cups from an Instant Pot). By weight the factor is about ×${QUINOA_COOKED_FACTOR}, worked out from USDA's dry and cooked entries, so <strong>${g2(gpc)} g of dry quinoa becomes roughly ${Math.round(gpc * QUINOA_COOKED_FACTOR)} g of cooked quinoa</strong>, and a cooked cup weighs ${QUINOA_COOKED_CUP} g (USDA; King Arthur says 184 g). That is heavier than a cooked cup of white rice (158 g) because quinoa holds more water — about 72% of its cooked weight. The "quinoa quadruples" line you may have read is a myth: three times by volume, three times by weight.</p>
<p>Working backwards: half a cup of <em>cooked</em> quinoa (about ${g2(QUINOA_COOKED_CUP / 2)} g) began as roughly ${Math.round(QUINOA_COOKED_CUP / 2 / QUINOA_COOKED_FACTOR)} g of dry seed, a scant 3 tablespoons. For planning, about 75 g of dry quinoa per person makes a main-course portion of ${Math.round(75 * QUINOA_COOKED_FACTOR)} g cooked, and 40–45 g a side. The <a href="/dry-to-cooked/">dry-to-cooked converter</a> runs any amount in either direction — including the "how much quinoa equals 250 g of cooked rice" kind of swap — and the <a href="/portion-calculator/">portion calculator</a> scales the dry grams to a table of any size. Weigh the quinoa <em>before</em> rinsing: rinsed seeds hold water and start absorbing it within minutes. Bob's Red Mill and Ancient Harvest both sell their quinoa pre-rinsed to remove the bitter saponin coating, though the Whole Grains Council still recommends a quick rinse for any residue.</p>
<h2>Calories, protein and the "48 g of whole grains" question</h2>
<p>A cup of dry quinoa (${g2(gpc)} g) carries about <strong>${Math.round(gpc * QUINOA_DRY_100.kcal / 100)} kcal, ${g2(gpc * QUINOA_DRY_100.protein / 100)} g of protein and ${g2(gpc * QUINOA_DRY_100.fiber / 100)} g of fiber</strong> (USDA: ${QUINOA_DRY_100.kcal} kcal, ${QUINOA_DRY_100.protein} g protein and ${QUINOA_DRY_100.fiber} g fiber per 100 g). Spread over the 3 cups it cooks into, that is <strong>about ${Math.round(QUINOA_COOKED_CUP * QUINOA_COOKED_100.kcal / 100)} kcal, ${g2(QUINOA_COOKED_CUP * QUINOA_COOKED_100.protein / 100)} g protein and ${g2(QUINOA_COOKED_CUP * QUINOA_COOKED_100.fiber / 100)} g fiber per cooked cup</strong>, or ${Math.round(QUINOA_COOKED_CUP / 2 * QUINOA_COOKED_100.kcal / 100)} kcal per half cup. The brand labels agree: Bob's Red Mill and Ancient Harvest both print 170 kcal and 6 g of protein for a quarter cup of dry quinoa. Quinoa is a seed cooked like a grain, and it counts as a whole grain because nothing is milled off — the germ and bran equivalent are all in the pot.</p>
<p>That is where the puzzling figure "48 grams of whole grains" comes from. It is not the weight of anything in a measuring cup: the Whole Grains Council's Whole Grain Stamp defines <strong>one serving as 16 g of whole-grain ingredient</strong>, and its "at least three servings a day" advice multiplies out to <strong>48 g a day</strong>. Since dry quinoa is 100% whole grain, half a cup of it (${g2(gpc / 2)} g) covers that whole day's target with room to spare, and half a cup of cooked quinoa — about ${Math.round(QUINOA_COOKED_CUP / 2 / QUINOA_COOKED_FACTOR)} g of grain once you subtract the water — is nearly two of the 16 g servings. The <a href="/grain-conversion-chart/">grain conversion chart</a> puts quinoa beside rice, oats, bulgur and barley for the same kinds of question, and the <a href="/recipe-weight-calculator/">recipe weight calculator</a> totals a mixed pot — 1 cup of rice plus 1/2 cup of quinoa is ${g2(ingBySlug("white-rice").gramsPerCup + gpc / 2)} g dry — in one go.</p>` : ""}${ing.slug === "cornstarch" ? `
...
<p><strong>Potato starch is the outlier</strong> — King Arthur weighs it at 152 g per cup, a third heavier than cornstarch, and Bob's Red Mill's label serving (1 tbsp = 12 g) implies heavier still, so weigh it rather than converting by cup. Arrowroot and tapioca sit within a few grams of cornstarch, so a recipe's cup measure carries over; what changes is behaviour, not weight — both thicken at a lower temperature than cornstarch, give a glossier, clearer gel, and turn stringy if boiled hard. Swapping cornstarch for flour is the one exchange that is not weight-for-weight or cup-for-cup: the <a href="/cornstarch-to-flour/">cornstarch to flour converter</a> handles the 1 : 2 volume rule (47% by weight) in both directions. Weighing a whole coating mix — flour, cornstarch, salt, pepper — in one bowl? The <a href="/recipe-weight-calculator/">recipe weight calculator</a> totals the list.</p>` : ""}${ing.slug === "oat-flour" ? `
<h2>92 g or 104 g? What a cup of oat flour weighs, by source</h2>
<p>Look up a cup of oat flour in two reference charts and you will get two numbers. <strong>King Arthur Baking's ingredient chart lists oat flour at 3 1/4 oz, or ${g2(gpc)} g per cup</strong> — the figure this page and its calculator use. <strong>USDA's FoodData Central measures the same level cup at ${OAT_FLOUR_USDA_CUP} g</strong> (oat flour, partially debranned, entry #169741). The difference is how the cup was filled: King Arthur spoons flour lightly into the cup and sweeps it level, while a cup dipped into the bag packs oat flour down — it is a fluffy, slightly oily powder that compacts more readily than wheat flour. The brand labels scatter around both figures because serving sizes are rounded to a convenient fraction and the nearest gram: King Arthur's own bag prints 1/3 cup = 28 g, Arrowhead Mills and Gold Medal 1/3 cup = 30 g, and Bob's Red Mill and Quaker 1/4 cup = 30 g.</p>
${oatFlourSourcesTable()}
<p>So <strong>half a cup of oat flour is about ${g2(gpc / 2)} g</strong>, a third of a cup ${g2(gpc / 3)} g and a quarter cup ${g2(gpc / 4)} g — and if another source gives 52, 35 or 26 g for the same measures, it is using USDA's cup, not a different flour. A quick sanity check for any figure you meet: a level US cup of oat flour lands between about <strong>85 and 120 g</strong>, with the two laboratory-measured values at ${g2(gpc)} and ${OAT_FLOUR_USDA_CUP} g. Anything above 120 g was packed or heaped (or is a 250 mL metric cup, which holds ${g2(gpc / 236.588 * 250)}–${g2(OAT_FLOUR_USDA_CUP / 236.588 * 250)} g), and a number near 160 g is <a href="/cups-to-grams/steel-cut-oats/">steel-cut oats</a>, not flour. <a href="/cups-to-grams/rolled-oats/">Rolled oats</a> themselves weigh almost the same as the flour ground from them, ${g2(ingBySlug("rolled-oats").gramsPerCup)} g a cup, because grinding fills the gaps between flakes but the powder stays fluffy.</p>
<h2>Oat flour in millilitres: 1/4 cup, 100 mL and metric cups</h2>
<p>A cup is a volume, so it converts to millilitres regardless of what is in it: <strong>1/4 US cup is 59 mL, 1/3 cup 79 mL, 1/2 cup 118 mL and a full cup 236.6 mL</strong>. What changes with the ingredient is the weight that volume holds. Oat flour is ${g2(gpc / 236.588 * 100)} g per 100 mL at this page's density, so <strong>1/4 cup (59 mL) of oat flour weighs about ${g2(gpc / 4)} g</strong>. Here are the common cup and glass sizes worked out both ways, with USDA's heavier fill alongside for anyone converting from a nutrition database:</p>
${oatFlourMlTable(gpc)}
<p class="note">Computed from ${g2(gpc)} g and ${OAT_FLOUR_USDA_CUP} g per 236.588 mL. A recipe from the UK, Australia or New Zealand means the 250 mL metric cup — about 6% more oat flour than a US cup — so weigh ${g2(gpc / 236.588 * 250)} g for "1 cup", not ${g2(gpc)} g. The <a href="/volume-converter/">volume converter</a> handles any other cup, glass or spoon size.</p>
<h2>Swapping oat flour for all-purpose flour: by weight, not by cup</h2>
<p>Oat flour is about <strong>${Math.round((1 - gpc / ingBySlug("all-purpose-flour").gramsPerCup) * 100)}% lighter than all-purpose flour</strong> per cup (${g2(gpc)} g against ${g2(ingBySlug("all-purpose-flour").gramsPerCup)} g), which is why a cup-for-cup swap comes out dry and crumbly: the recipe gets a quarter less flour than it expects. The mills agree on the fix. <strong>Quaker's Oat Flour 101 guide: substitute "at a 1:1 ratio by weight", or by volume use "1 1/4 cup leveled unpacked oat flour to replace 1 cup leveled unpacked all-purpose flour"</strong>; Arrowhead Mills says oat flour "should be used by equal weight instead of volume". Both rules are the same arithmetic — ${g2(ingBySlug("all-purpose-flour").gramsPerCup)} g of oat flour is ${cups2(ingBySlug("all-purpose-flour").gramsPerCup / gpc)} cups — done here for every common amount:</p>
${oatFlourSwapTable(gpc, ingBySlug("all-purpose-flour").gramsPerCup)}
<p>Weight solves the measuring problem, not the structural one. Oat flour has no gluten, so it cannot hold the gas that makes bread rise: Quaker suggests replacing only about a third of the flour in anything that rises, King Arthur uses oat flour for "a portion (not all)" of the all-purpose flour in tender bakes like muffins, cakes and cookies and rates its own oat flour for up to half the flour in non-yeasted recipes, and Bob's Red Mill recommends adding xanthan gum when the swap is total. For the other direction — how much all-purpose, whole wheat or almond flour stands in for oat flour — the <a href="/ingredient-substitution-chart/">ingredient substitution chart</a> runs the same by-weight logic across the whole flour shelf, and the <a href="/flour-conversion-chart/">flour conversion chart</a> puts every flour's cup weight side by side. A blend of two flours is easiest to total in the <a href="/recipe-weight-calculator/">recipe weight calculator</a>.</p>
<h2>Homemade oat flour, calories and the gluten-free question</h2>
<p>Grinding your own is a 30-second job in a blender, and the scale makes it exact: <strong>weight does not change when flakes become powder</strong>, so for "1 cup oat flour" grind ${g2(gpc)} g of <a href="/cups-to-grams/rolled-oats/">rolled oats</a> — a cup of oats is ${g2(ingBySlug("rolled-oats").gramsPerCup)} g, so one cup of oats makes almost exactly one cup of flour by weight. None of the big mills publish a cup-for-cup yield, because freshly ground flour settles unevenly, which is why King Arthur's recipes give the oats as a weight ("1 1/2 cups (134 g) old-fashioned rolled oats, ground for 30 seconds"). Old-fashioned, quick or instant oats all work; Quaker mills its retail oat flour from the same three.</p>
<p>On the nutrition side, USDA lists oat flour at <strong>${OAT_FLOUR_100.kcal} kcal per 100 g</strong>, with ${OAT_FLOUR_100.protein} g protein, ${OAT_FLOUR_100.fat} g fat and ${OAT_FLOUR_100.fiber} g fiber — so a spooned cup (${g2(gpc)} g) is about <strong>${Math.round(gpc * OAT_FLOUR_100.kcal / 100)} kcal</strong>, half a cup ${Math.round(gpc / 2 * OAT_FLOUR_100.kcal / 100)} kcal, and King Arthur's label (110 kcal per 28 g) agrees. Whole-grain oat flour keeps more of the bran than USDA's "partially debranned" entry, and its Foundation Foods entry shows the difference: 10.5 g of fiber per 100 g. Finally, gluten: oats have none, but the Celiac Disease Foundation warns that they are routinely grown, stored and shipped alongside wheat, barley and rye, and advises buying only oat products labelled gluten-free. Quaker's guide says plainly that its oat flour is not certified; Bob's Red Mill mills in a dedicated gluten-free facility and King Arthur's is GFCO-certified. None of that changes the weight — certified or not, a spooned cup is ${g2(gpc)} g.</p>` : ""}${ing.slug === "grated-parmesan" ? `
<h2>100 g, 80 g or 120 g? What a cup of grated parmesan weighs, by source</h2>
<p>Look up a cup of grated parmesan in the references and you will get three answers, and none of them is a typo. <strong>King Arthur Baking's ingredient chart lists grated Parmesan at 1/2 cup = 1 3/4 oz = 50 g</strong>, so ${g2(gpc)} g per cup &mdash; the figure this page and its calculator use. <strong>USDA FoodData Central measures the same level cup at ${PARM_USDA_CUP} g</strong> (#171247, "Cheese, parmesan, grated"). But the <strong>nutrition labels</strong> tell a different story: Kraft's green shaker prints <strong>2 teaspoons = 5 g</strong>, which multiplies out to ${PARM_KRAFT_SHAKER_CUP} g per cup, while BelGioioso and 4C print <strong>1 tablespoon = 5 g</strong>, which is only ${5 * 16} g per cup &mdash; and USDA's own record carries that same 5 g tablespoon beside its 100 g cup. Here is every published figure side by side, with America's Test Kitchen's grate-size yields underneath:</p>
${parmSourcesTable()}
<p>So <strong>half a cup of grated parmesan is ${g2(gpc / 2)} g</strong>, a third of a cup ${g2(gpc / 3)} g, a quarter cup ${g2(gpc / 4)} g and a level tablespoon ${g2(gpc / 16)} g on this page's fine grate &mdash; and if a recipe gives you 1 1/4 cups, weigh ${g2(gpc * 1.25)} g. The spread in the table is not measurement error. A tablespoon is filled loosely, so the 5 g label spoon is real; a cup settles as it fills, so USDA's 100 g cup is real too; and the shaker's dry, evenly milled cheese packs denser than anything that came off a grater at home. If a US recipe says "1 cup grated parmesan" with no other clue, ${g2(gpc)} g is the safe reading; if it was written with the shaker in mind, ${PARM_KRAFT_SHAKER_CUP} g; if it says "finely grated on a rasp grater", read on.</p>
<h2>Grate size decides the weight: Microplane, box grater, shaker</h2>
<p>America's Test Kitchen weighs this out plainly: <strong>the same 1 ounce of hard cheese makes 1/2 cup grated fine on a rasp-style grater, 1/3 cup on the medium holes of a box grater, or 1/4 cup grated coarse</strong> &mdash; a two-to-one range from one wedge. Turned into cup weights that is about <strong>${Math.round(parmGrateCup(0.5))} g per cup from a Microplane, ${Math.round(parmGrateCup(1 / 3))} g medium-grated and ${Math.round(parmGrateCup(0.25))} g coarse</strong>, against ${g2(gpc)} g for the fine canister grade. A cup of fluffy Microplaned Parmigiano can hold barely half the cheese of a cup from the shaker, which is why test kitchens print the grate size next to the weight. This table runs it the other way, from the wedge in your hand to the cups you will get:</p>
${parmWedgeTable()}
<p class="note">Cups per ounce from America's Test Kitchen's "Grating Cheese Weight and Volume"; the last column divides by this page's ${g2(gpc)} g. A "cubic inch" of hard parmesan is 10.3 g (USDA #170848), so a 1-inch cube gives just under 3 tablespoons Microplaned. The weight of the cheese never changes when you grate it &mdash; only the volume does &mdash; so a recipe that says "4 oz parmesan, grated" wants ${g2(4 * OZ)} g however you grate it, while "1 cup grated parmesan" wants somewhere between ${Math.round(parmGrateCup(0.5))} and ${PARM_KRAFT_SHAKER_CUP} g depending on the grater. When you need to hit a weight, grate onto the scale.</p>
<h2>Parmesan in tablespoons, millilitres and metric cups</h2>
<p>A cup is a volume, so it converts to millilitres regardless of what is in it: a US tablespoon is 14.8 mL, <strong>1/4 US cup is 59 mL, 1/2 cup 118 mL and a full cup 236.6 mL</strong>, while the metric cup used in the UK, Australia and New Zealand is 250 mL &mdash; about 6% more, so ${g2(gpc / 236.588 * 250)} g of fine grated parmesan. Because the weight per cup depends so much on the grate, this table gives each measure three ways: the ${g2(gpc)} g fine grate (this page), the Kraft shaker's ${PARM_KRAFT_SHAKER_CUP} g density and the fluffy Microplane cup at about ${Math.round(parmGrateCup(0.5))} g.</p>
${parmMlTable(gpc)}
<p class="note">Computed from ${g2(gpc)} g, ${PARM_KRAFT_SHAKER_CUP} g and ${g2(parmGrateCup(0.5))} g per 236.588 mL. The label's "1 tbsp = 5 g" is lighter than every tablespoon in this table because a spoon is filled loosely and heaped off; the shaker cup is heavier because milled cheese settles. The <a href="/tablespoons-to-grams/">tablespoons to grams</a> page handles other spoon amounts, and the <a href="/grams-to-cups/">grams to cups</a> converter goes the other way.</p>
<h2>Parmigiano Reggiano, calories and sodium</h2>
<p>For the scale, <strong>parmesan is parmesan</strong>: Parmigiano Reggiano, Grana Padano, domestic block "parmesan" and Pecorino Romano all weigh about the same per cup at the same grate size, so every figure above applies to all of them. What differs is the cheese itself. <strong>Parmigiano Reggiano is a PDO (protected designation of origin) product</strong> made only in Parma, Reggio Emilia, Modena and parts of Bologna and Mantua, matured for <strong>at least 12 months</strong> &mdash; most of what is sold is 24 to 36 months old &mdash; and under its product specification even the grated version must be grated inside the region, with 25&ndash;35% moisture and no more than 18% rind by weight. Older, drier wheels grate finer and fluffier, which nudges a home-grated cup toward the light end of the range; young domestic parmesan and the canister products sit at the heavy end.</p>
<p>On the nutrition side USDA lists grated parmesan at <strong>${PARM_100.kcal} kcal per 100 g</strong> with ${PARM_100.protein} g protein and ${PARM_100.fat} g fat, so a level ${g2(gpc)} g cup is about <strong>${Math.round(gpc * PARM_100.kcal / 100)} kcal</strong>, a half cup ${Math.round(gpc / 2 * PARM_100.kcal / 100)} kcal and the label's 5 g tablespoon about ${Math.round(5 * PARM_100.kcal / 100)} kcal &mdash; the 20 kcal Kraft, BelGioioso and 4C print, after rounding. USDA's shredded entry (#173431) is similar at ${PARM_SHRED_100.kcal} kcal but far higher in protein (${PARM_SHRED_100.protein} g per 100 g), and a solid wedge (#170848) ${PARM_HARD_100.kcal} kcal with ${PARM_HARD_100.protein} g protein. The number worth watching is salt: <strong>${PARM_100.sodium} mg sodium per 100 g</strong>, so a cup of grated parmesan carries about ${(gpc * PARM_100.sodium / 100 / 1000).toFixed(1)} g of sodium &mdash; roughly three-quarters of a teaspoon of table salt &mdash; alongside ${Math.round(gpc * PARM_100.calcium / 100)} mg of calcium. Comparing it with other cheeses by the cup? The <a href="/dairy-conversion-chart/#cheese-equivalents">dairy conversion chart</a> lines up shredded, crumbled and soft cheeses, and the <a href="/recipe-weight-calculator/">recipe weight calculator</a> totals a whole pasta or risotto ingredient list in grams.</p>` : ""}${ing.slug === "chopped-nuts" ? `
...
// Dairy reference weights verified first-hand against USDA FoodData Central (SR Legacy),
// 2026-09-01, used by the dairy chart's sanity-check section and FAQs. Kept separate from
// ingredients.json because they are alternative STATES of an ingredient (whipped, frozen,
// stick, label serving), not cup weights of the ingredient the chart lists.
const DAIRY_REF = {
  creamFluid: 238,    // #170859 heavy whipping cream, 1 cup fluid (yields 2 cups whipped)
  creamWhipped: 120,  // #170859, 1 cup whipped
  butterStick: 113,   // #173430, 1 stick (= 1/2 cup); 1 cup 227 g; 1 tbsp 14.2 g
  butterTbsp: 14.2,
  milkUsda: 244,      // #171265 whole milk, 1 cup (this chart uses a mid-source 240 g)
  oilTbsp: 13.6,      // #171411 soybean/vegetable oil, 1 tbsp; 1 cup 218 g
  oilLabelTbsp: 14,   // the rounded per-tablespoon figure printed on US oil labels
  iceCreamCup: 132,   // #167575 vanilla ice cream, 1/2 cup 66 g
};
...
function categoryPage(key) {
  ...
  const wAny = (slug) => (DATA.ingredients.find((i) => i.slug === slug) || {}).gramsPerCup;
  ...
  const description = key === "sugar"
    ...
    : key === "dairy"
    ? `Free dairy conversion chart: grams per cup for butter, milk, cream, yogurt, oil and every cheese — plus every cup fraction (half a cup of milk is ${g2(w("milk") / 2)} g, a stick of butter ${DAIRY_REF.butterStick} g), a cheese cup equivalents table (4 oz block = 1 cup shredded = 113 g), and how to sanity-check a dairy weight that looks wrong.`
    : key === "baking"
    ? `Free baking conversion chart: grams per cup for chocolate chips, nuts and seeds — plus a 100 g in cups reverse table (100 g of almonds ≈ 0.7 cups, chopped nuts ≈ 0.83, flaxseed = 1 cup), every cup fraction (half a cup of chocolate chips is ${g2(w("chocolate-chips") / 2)} g), and how to sanity-check a nut or chocolate weight that looks wrong.`
    : key === "grain"
    ? `Free grain conversion chart: grams per cup for rice, quinoa, oats, couscous, bulgur and pearl barley — plus every cup fraction (half a cup of rice is ${g2(w("white-rice") / 2)} g), a 100 g reverse lookup, how to sanity-check a grain weight that looks wrong, and whole-grain servings per cup.`
    : key === "flour"
  ...
  const faq = (CATEGORY_FAQ[key] || []).slice();
  // Grain FAQ extras are computed rather than typed, because they quote a lot of
  // numbers (every fraction of every grain) that must track ingredients.json.
  ...
  if (key === "baking") {
    // Baking FAQ extras are computed from ingredients.json / NUT_FORMS so they can
    // never disagree with the fraction and sanity-check tables. Question shapes are
    // observed Bing queries on this page: "half cup of chocolate chips in grams",
    // "is 145 grammes of chocolate peanuts a lot" (clicked, pos 4), "1/2 cup nuts
    // and seeds into grams" and the chopped-vs-whole verification shape.
    faq.push(
      ["How many grams is half a cup of chocolate chips?", `About ${g2(w("chocolate-chips") / 2)} g - half of the ${g2(w("chocolate-chips"))} g in a full US cup, and the same for white chocolate chips. A third of a cup is about ${g2(w("chocolate-chips") / 3)} g, a quarter cup ${g2(w("chocolate-chips") / 4)} g and a tablespoon about ${g2(w("chocolate-chips") / 16)} g. Mini chips run slightly heavier (USDA measures 173 g per cup, so about ${g2(173 / 2)} g per half cup) and chunks heavier still at 182 g per cup.`],
      ["Is 145 grams of chocolate-covered peanuts a lot?", `As a quantity it is about one US cup - ${g2(145 / OZ)} oz, or roughly five of the 1 oz (28 g) handfuls that nut packaging calls a serving. USDA weighs a cup of raw shelled peanuts at ${nutG("peanut-raw")} g and a cup of chocolate chips is ${g2(w("chocolate-chips"))} g, so a cup of chocolate-coated peanuts lands in the same region and 145 g is roughly a level cup. Folded into a batch of cookies that is an ordinary mix-in amount; as a snack it is five servings.`],
      ["Do nuts weigh more chopped or whole?", `It depends on the shape of the whole nut. Big curved halves get heavier per cup when chopped because the pieces fill the gaps - USDA weighs walnut halves at ${nutG("walnut-halves")} g and chopped walnuts at ${nutG("walnut-chopped")} g, pecan halves ${nutG("pecan-halves")} g against ${nutG("pecan-chopped")} g chopped. Compact round nuts go the other way - whole hazelnuts ${nutG("hazelnut-whole")} g against ${nutG("hazelnut-chopped")} g chopped, whole almonds ${nutG("almond-whole")} g against ${nutG("almond-slivered")} g slivered and ${nutG("almond-sliced")} g sliced. Ground nuts are lightest of all, ${nutG("hazelnut-ground")}-${nutG("almond-ground")} g a cup.`],
      ["Which nuts and seeds weigh the most per cup?", `Of everything on this chart, dried fruit included, chia seeds at ${g2(w("chia-seeds"))} g a cup, then raisins ${g2(w("raisins"))} g, poppy seeds ${g2(w("poppy-seeds"))} g, and whole almonds, hazelnuts, pine nuts and sesame seeds all at ${g2(w("whole-almonds"))} g. The lightest are shredded coconut at ${g2(w("shredded-coconut"))} g, ground flaxseed ${g2(w("ground-flaxseed"))} g and pecan halves ${g2(w("pecan-halves"))} g. Chocolate chips outweigh all of them at ${g2(w("chocolate-chips"))} g, so a 12 oz bag of chips is 2 cups while 12 oz of pecan halves is over 3 cups.`],
      ["How many grams is a third of a cup of nuts or chocolate chips?", `A third of a cup of chocolate chips is about ${g2(w("chocolate-chips") / 3)} g, of chopped nuts ${g2(w("chopped-nuts") / 3)} g, of whole almonds or hazelnuts ${g2(w("whole-almonds") / 3)} g, of chopped walnuts or cashews ${g2(w("walnuts-chopped") / 3)} g, of pecan halves ${g2(w("pecan-halves") / 3)} g, of raisins ${g2(w("raisins") / 3)} g and of shredded coconut ${g2(w("shredded-coconut") / 3)} g. A third of a cup is 5 tablespoons plus 1 teaspoon, which is why it is so awkward to measure and worth weighing.`],
    );
  }
  if (key === "grain") {
    const gr = (slug) => g2(w(slug));
    faq.push(
      ["How many grams is half a cup of rice?", `About ${g2(w("white-rice") / 2)} g of uncooked long-grain white rice — half of the ${gr("white-rice")} g in a full US cup. Half a cup of the other grains on this chart: quinoa ${g2(w("quinoa") / 2)} g, couscous ${g2(w("couscous-dry") / 2)} g, bulgur ${g2(w("bulgur-dry") / 2)} g, steel-cut oats ${g2(w("steel-cut-oats") / 2)} g, pearl barley ${g2(w("pearl-barley") / 2)} g, cornmeal ${g2(w("cornmeal") / 2)} g and rolled oats ${g2(w("rolled-oats") / 2)} g. All dry weights — the fraction table on this page has quarters, thirds and two-thirds as well.`],
      ["How many cups is 100 g of rice?", `About ${cups2(100 / w("white-rice"))} of a cup — just over half a cup of uncooked long-grain white rice. The same 100 g is ${cups2(100 / w("quinoa"))} cups of quinoa, ${cups2(100 / w("bulgur-dry"))} cups of bulgur, ${cups2(100 / w("pearl-barley"))} of a cup of pearl barley and a generous ${cups2(100 / w("rolled-oats"))} cups of rolled oats. The reverse-lookup table above has 50 g and 250 g too.`],
      ["Is there 48 grams of whole grains in half a cup of quinoa?", `Not as a weight in the cup. Half a US cup of dry quinoa is ${g2(w("quinoa") / 2)} g, and half a cup of cooked quinoa about ${g2(185 / 2)} g (USDA weighs a cooked cup at 185 g); 48 g of dry quinoa is only about ${cups2(48 / w("quinoa"))} of a cup, roughly ${g2(48 / w("quinoa") * 16)} tablespoons. The figure 48 comes from labelling rather than measuring: the Whole Grains Council's 100% Stamp defines one whole-grain serving as 16 g of whole grain, so 48 g is three of those servings. Both are quoted in grams, which is exactly why they get confused.`],
      ["Does a cup of quinoa weigh the same as a cup of rice?", `No — a cup of dry quinoa is ${gr("quinoa")} g against ${gr("white-rice")} g for uncooked long-grain white rice, so quinoa is about ${g2(w("white-rice") - w("quinoa"))} g lighter per cup (${Math.round((1 - w("quinoa") / w("white-rice")) * 100)}%). Quinoa's seeds are smaller and rounder and leave more space between them. Cooked, the order reverses: USDA weighs a cup of cooked quinoa at 185 g against 158 g for cooked white rice, because quinoa absorbs more water.`],
      ["Which grain weighs the most per cup?", `Of the grains on this chart, pearl barley at ${gr("pearl-barley")} g a cup, then uncooked white rice at ${gr("white-rice")} g and couscous at ${gr("couscous-dry")} g. The lightest entries are panko breadcrumbs at ${gr("panko-breadcrumbs")} g and rolled oats at ${gr("rolled-oats")} g — panko is barely a quarter the weight of pearl barley for the same cup. Table salt (${gr("salt")} g) and water (${gr("water")} g) are heavier still, but neither is a grain.`],
      ["How many grams is a quarter cup of grain?", `A quarter cup of uncooked white rice is ${g2(w("white-rice") / 4)} g, quinoa ${g2(w("quinoa") / 4)} g, couscous ${g2(w("couscous-dry") / 4)} g, bulgur ${g2(w("bulgur-dry") / 4)} g, pearl barley ${g2(w("pearl-barley") / 4)} g, cornmeal ${g2(w("cornmeal") / 4)} g, steel-cut oats ${g2(w("steel-cut-oats") / 4)} g and rolled oats ${g2(w("rolled-oats") / 4)} g. A quarter cup is 4 tablespoons, so a single tablespoon of rice is about ${g2(w("white-rice") / 16)} g.`],
      ["Should I weigh grains dry or cooked?", `Dry, unless the recipe plainly says otherwise — recipes, package directions and this chart are all written for dry weight, and dry weight is far more repeatable. Cooked weight swings with the water ratio, the pot, the lid and how long the grain sits afterwards, which is why published cooked yields disagree with each other. Weigh dry and before rinsing, too: rinsed rice holds surface and absorbed water, so a drained cup is no longer ${gr("white-rice")} g.`],
      ["Why do grain charts disagree about how much a cup of rice weighs?", `Usually because they are not describing the same thing. Three differences account for nearly all of it: the cup (a US cup is 236.6 mL, the UK/Australian/New Zealand metric cup 250 mL, and a rice-cooker cup only 180 mL — the same long-grain rice is ${gr("white-rice")} g, ${Math.round(w("white-rice") / 236.588 * 250)} g or ${Math.round(w("white-rice") / 236.588 * 180)} g in those three); the rice (long-grain ${gr("white-rice")} g, medium-grain about 195 g, short-grain or sushi about 200 g); and dry versus cooked (USDA weighs a cup of cooked white rice at 158 g). A chart that differs by 10–15 g is almost always answering a slightly different question rather than being wrong.`],
      ["How many whole-grain servings are in a cup of oats or quinoa?", `A cup of dry rolled oats (${gr("rolled-oats")} g) is about ${(w("rolled-oats") / OATS_SERVE_WGC_G).toFixed(1)} Whole Grains Council servings and a cup of dry quinoa (${gr("quinoa")} g) about ${(w("quinoa") / OATS_SERVE_WGC_G).toFixed(1)}, because both are 100% whole grain and the Council counts 16 g of whole-grain ingredient as one serving. In USDA ounce-equivalents (1 oz, or ${g2(OZ)} g, of dry grain each) that is ${(w("rolled-oats") / OZ).toFixed(1)} for the oats and ${(w("quinoa") / OZ).toFixed(1)} for the quinoa. Half a cup of uncooked oats therefore holds ${g2(w("rolled-oats") / 2)} g of whole grain — labels print 40 g only because Quaker's half-cup serving weighs 40 g.`],
      ["How many grams is a quarter cup of whole-grain rice?", `About ${g2(w("white-rice") / 4)} g dry. Brown long-grain rice weighs the same per cup as white (USDA: ${gr("white-rice")} g), so a quarter cup of either is ${g2(w("white-rice") / 4)} g; the difference is what counts. Brown rice is a whole grain, so that quarter cup is about ${(w("white-rice") / 4 / OATS_SERVE_WGC_G).toFixed(1)} Whole Grains Council servings (16 g each) and ${(w("white-rice") / 4 / OZ).toFixed(1)} USDA ounce-equivalents, while white rice, with the bran and germ milled off, contributes no whole grain at all. Cooked, a quarter cup of brown rice becomes roughly ${Math.round(w("white-rice") / 4 * 2.98)} g.`],
      ["Does pearl barley or white rice count as a whole grain?", `No. Pearling scrapes off barley's bran (and some of the germ), and white rice has its bran and germ milled away, so under the Whole Grains Council's definitions neither counts however many grams you cook; hulled barley and brown rice do. Their cup weights are unaffected — pearl barley is still ${gr("pearl-barley")} g and white rice ${gr("white-rice")} g. Cornmeal is the ambiguous one: ordinary degerminated cornmeal is refined, while a bag labelled whole-grain or stone-ground counts in full, about ${(w("cornmeal") / OATS_SERVE_WGC_G).toFixed(1)} servings per ${gr("cornmeal")} g cup.`],
    );
  }
  if (key === "dairy") {
    // Dairy FAQ extras are computed from ingredients.json / DAIRY_REF so they can never
    // disagree with the fraction and sanity-check tables. Question shapes are observed
    // Bing queries on this chart and its ingredient pages: "1 cup milk in grams",
    // "1/8 cup of olive oil", "weight of 1 cup vegetable oil that weighs 224g?",
    // the butter-stick cluster ("1 1/2 sticks of butter to cups", "130g butter to
    // sticks"), "how many cups is 454 g of ricotta", and the metric-cup variants
    // ("... in grams australia", "... canada", "... uk").
    const R = DAIRY_REF, MC = 250 / 236.588;
    faq.push(
      ["How many grams is half a cup of milk?", `About ${g2(w("milk") / 2)} g. Half a cup of the other pourable and spoonable dairy on this chart is within a few grams of that: buttermilk ${g2(w("buttermilk") / 2)} g, heavy cream ${g2(w("heavy-cream") / 2)} g, sour cream ${g2(w("sour-cream") / 2)} g, yogurt ${g2(w("yogurt") / 2)} g, cream cheese ${g2(w("cream-cheese") / 2)} g — and half a cup of butter is ${g2(w("butter") / 2)} g, which is one US stick. The fraction table above has every amount from a tablespoon to a full cup for all ${items.length} ingredients.`],
      ["Is a cup of milk 8 ounces?", `By volume, yes — a US cup is 8 fluid ounces. By weight it is not: a cup of milk weighs about ${g2(w("milk"))} g, which is ${g2(w("milk") / OZ)} oz on a scale (USDA's measured cup, ${R.milkUsda} g, is ${g2(R.milkUsda / OZ)} oz). The two kinds of ounce never quite coincide — even a cup of water weighs ${g2(wAny("water") / OZ)} oz — but butter comes closest, by a coincidence of density: 1 cup = ${g2(w("butter"))} g = ${g2(w("butter") / OZ)} oz = 8 fl oz. A cup of oil is 8 fl oz but only ${g2(w("vegetable-oil") / OZ)} oz by weight; a cup of sweetened condensed milk is 8 fl oz but ${g2(w("sweetened-condensed-milk") / OZ)} oz. If a recipe says "8 oz of milk" it almost always means the fluid measure — a cup.`],
      ["How many grams is 1/8 cup of olive oil?", `About ${g2(w("olive-oil") / 8)} g — an eighth of a cup is exactly 2 tablespoons, so it is also 2 × ${g2(w("olive-oil") / 16)} g. The same 2 tablespoons of vegetable or coconut oil is ${g2(w("vegetable-oil") / 8)} g, of melted butter ${g2(w("butter") / 8)} g, and of milk ${g2(w("milk") / 8)} g. Oil is the one family on this chart that is lighter than water (about 0.92 g per mL), so an oil figure that comes out heavier than the same volume of milk has been mixed up with something else.`],
      ["Is a cup of vegetable oil 218 g or 224 g?", `Both figures are in circulation and both are honest. ${g2(w("vegetable-oil"))} g is the measured cup (USDA weighs a tablespoon of soybean/vegetable oil at ${R.oilTbsp} g, and 16 of those is ${g2(R.oilTbsp * 16)} g). 224 g is label arithmetic: US Nutrition Facts panels round the tablespoon to ${R.oilLabelTbsp} g, and 16 × ${R.oilLabelTbsp} = ${R.oilLabelTbsp * 16}. The gap is under 3%, smaller than the difference between filling a cup to the line and slightly over it, so either number is fine for cooking. This chart uses ${g2(w("vegetable-oil"))} g for vegetable and coconut oil and ${g2(w("olive-oil"))} g for olive oil.`],
      ["How much does a cup of whipped cream weigh?", `About ${R.creamWhipped} g — half the weight of the liquid cream it came from. USDA measures a cup of fluid heavy whipping cream at ${R.creamFluid} g and notes that it yields 2 cups whipped, each weighing ${R.creamWhipped} g: whipping doubles the volume by folding in air and adds no weight. So if a recipe wants "1 cup heavy cream, whipped", measure ${g2(w("heavy-cream"))} g of liquid cream and whip it; if it wants "1 cup of whipped cream", that is only about ${R.creamWhipped} g of liquid cream to start with, roughly half a cup.`],
      ["How many sticks of butter is 1 1/2 cups?", `Three sticks. A US stick of butter is 1/2 cup, ${R.butterStick} g (4 oz, 8 tablespoons), so 3/4 cup is 1 1/2 sticks (${g2(R.butterStick * 1.5)} g), 1 cup is 2 sticks (${g2(w("butter"))} g), 1 1/2 cups is 3 sticks (${g2(R.butterStick * 3)} g) and 2 cups is 4 sticks, a full 1 lb box. Going the other way, 130 g of butter is ${Math.round(130 / R.butterStick * 100) / 100} sticks — one stick plus a heaped tablespoon (${g2(130 - R.butterStick)} g). Note that sticks are a US format: a European 250 g block is ${cups2(250 / w("butter"))} cups, not one. The butter converter handles any amount in sticks, cups, tablespoons and grams.`],
      ["How many cups is 454 g (1 lb) of ricotta cheese?", `About 2 cups on this chart's ${g2(w("ricotta-cheese"))} g per cup (King Arthur's figure) — a 1 lb tub is ${cups2(454 / w("ricotta-cheese"))} cups, and the common 15 oz (425 g) tub about ${cups2(425 / w("ricotta-cheese"))} cups. USDA measures ricotta more firmly packed at 246 g per cup, which would make the pound tub ${cups2(454 / 246)} cups, so the honest answer is "a little under to a little over 2 cups depending on how you fill the cup". The same 454 g is ${cups2(454 / w("cottage-cheese"))} cups of cottage cheese, ${cups2(454 / w("cream-cheese"))} cups of cream cheese, and ${cups2(454 / w("shredded-cheddar"))} cups of shredded cheddar.`],
      ["How much does a metric cup of milk weigh?", `About ${Math.round(w("milk") * MC)} g. The metric cup used in Australia, New Zealand, the UK and Canada holds 250 mL against the US cup's 236.6 mL, so every weight on this chart is about ${Math.round((MC - 1) * 1000) / 10}% higher in a metric cup: heavy cream ${Math.round(w("heavy-cream") * MC)} g, yogurt ${Math.round(w("yogurt") * MC)} g, butter ${Math.round(w("butter") * MC)} g, coconut oil ${Math.round(w("coconut-oil") * MC)} g and grated parmesan ${Math.round(w("grated-parmesan") * MC)} g. Half a metric cup of heavy cream is therefore about ${Math.round(w("heavy-cream") * MC / 2)} g rather than ${g2(w("heavy-cream") / 2)} g. This chart is US cups throughout.`],
    );
  }
...
<h2 id="grain-cup-fractions">Grains to grams: every cup fraction</h2>
<p>The chart above gives three amounts; recipes ask for more than three. This table has every fraction a grain recipe actually calls for &mdash; three-quarters, two-thirds, a third &mdash; plus a single tablespoon, for all ${items.length} entries on the page. The three looked up most: <strong>1 cup of uncooked white rice is ${g2(w("white-rice"))}&nbsp;g and half a cup ${g2(w("white-rice") / 2)}&nbsp;g; 1 cup of quinoa is ${g2(w("quinoa"))}&nbsp;g and half a cup ${g2(w("quinoa") / 2)}&nbsp;g; 1 cup of rolled oats is ${g2(w("rolled-oats"))}&nbsp;g and half a cup ${g2(w("rolled-oats") / 2)}&nbsp;g.</strong> All dry, all levelled off in a US cup.</p>
<div class="tw"><table><thead><tr><th>Grain</th><th>1 cup</th><th>&frac34; cup</th><th>&frac23; cup</th><th>&frac12; cup</th><th>&frac13; cup</th><th>&frac14; cup</th><th>1 tbsp</th></tr></thead><tbody>${
  items.map((i) => `<tr><td><a href="/cups-to-grams/${i.slug}/">${esc(i.name)}</a></td>` +
    [1, 0.75, 2 / 3, 0.5, 1 / 3, 0.25, 1 / 16].map((f) => `<td class="num">${g2(i.gramsPerCup * f)} g</td>`).join("") +
    `</tr>`).join("")
}</tbody></table></div>
<p class="note">Every cell is computed from the same verified grams-per-cup weight as the chart above (&frac34; cup = weight &times; 0.75, 1 tbsp = weight &divide; 16), so the two tables can never disagree. For an amount that is not in the table, the converter at the top of the page takes any number, and <a href="/tablespoons-to-grams/">tablespoons to grams</a> handles spoon amounts.</p>
<h2 id="grain-100g">100 g of grain in cups (reverse lookup)</h2>
<p>The other direction: a weight from a recipe, a bag or a scale, and only measuring cups to hand. Because every grain packs differently, <strong>100&nbsp;g is a different number of cups for each one</strong> &mdash; ${cups2(100 / w("rolled-oats"))} cups of rolled oats but only ${cups2(100 / w("pearl-barley"))} of a cup of pearl barley, for the same weight. The useful anchor: most dry grains sit near <strong>half a cup per 100&nbsp;g</strong>, and flaked or crumbed things at roughly double that.</p>
<table><thead><tr><th>Grain</th><th>50 g in cups</th><th>100 g in cups</th><th>250 g in cups</th></tr></thead><tbody>${
  items.slice().sort((a, b) => a.gramsPerCup - b.gramsPerCup)
    .map((i) => `<tr><td><a href="/cups-to-grams/${i.slug}/">${esc(i.name)}</a></td><td class="num">${cups2(50 / i.gramsPerCup)}</td><td class="num">${cups2(100 / i.gramsPerCup)}</td><td class="num">${cups2(250 / i.gramsPerCup)}</td></tr>`).join("")
}</tbody></table>
<p class="note">Computed as grams &divide; the grams-per-cup weight in the chart above, so the two always agree. For any other weight, the <a href="/grams-to-cups/">grams to cups converter</a> takes a number directly &mdash; every grain here is in its dropdown. Adding more than one grain together, as in a mixed pilaf? The <a href="/recipe-weight-calculator/">recipe weight calculator</a> totals the whole list at once.</p>
<h2 id="grain-sanity-check">Is that grain weight right? A ten-second sanity check</h2>
<p>Plenty of people arrive at a chart like this one not to convert a number but to <em>check</em> one &mdash; a figure from a recipe, a package or a tracking app that looks too big or too small. Two steps settle almost all of them.</p>
<p><strong>Step 1: check the band.</strong> Dry grains are not scattered at random. Everything on this chart falls into one of three groups, and simply knowing which group you are in catches an implausible number immediately.</p>
<table><thead><tr><th>Group</th><th>1 cup weighs</th><th>What is in it</th></tr></thead><tbody>${
  [["Flakes and crumbs", (gr) => gr < 110],
   ["Meals, cracked and whole grains", (gr) => gr >= 110 && gr < 200],
   ["Dense fills", (gr) => gr >= 200]]
    .map(([label, test]) => {
      const ms = items.filter((i) => test(i.gramsPerCup)).sort((a, b) => a.gramsPerCup - b.gramsPerCup);
      return `<tr><td><strong>${label}</strong></td><td class="num">${g2(ms[0].gramsPerCup)}&ndash;${g2(ms[ms.length - 1].gramsPerCup)} g</td><td>${ms.map((i) => `<a href="/cups-to-grams/${i.slug}/">${esc(i.name)}</a> ${g2(i.gramsPerCup)}&nbsp;g`).join(" &middot; ")}</td></tr>`;
    }).join("")
}</tbody></table>
<p class="note">Group boundaries and every weight are read straight from the chart above &mdash; nothing here is typed by hand.</p>
<p>Flakes and crumbs are mostly air, so they are light and easy to over-pack: if a flake or crumb is coming out much above ${g2(w("breadcrumbs"))}&nbsp;g a cup, it has been pressed down or the number is not for the dry version. The middle group is where nearly every named grain lives, and it is narrower than people expect &mdash; <strong>every named dry grain on this chart sits between ${g2(w("cornmeal"))} and ${g2(w("white-rice"))}&nbsp;g a cup, a ${Math.round(w("white-rice") / w("cornmeal") * 10) / 10}&times; spread end to end</strong>. So a rice-sized number for a quinoa-sized grain is fine; a doubled one is not. Above 200&nbsp;g you are into things that leave almost no space at all: <a href="/cups-to-grams/water/">water</a> is the natural reference at ${g2(w("water"))}&nbsp;g a cup, and only a solid that packs tighter than water &mdash; <a href="/cups-to-grams/salt/">table salt</a> at ${g2(w("salt"))}&nbsp;g &mdash; goes above it.</p>
<p><strong>Step 2: check which question the number is answering.</strong> A grain weight that fails the band test is usually not wrong, just an answer to a different question. Three possibilities, worth ruling out in this order.</p>
<p><em>Dry or cooked?</em> By far the most common mix-up, and the biggest. Cooking multiplies a grain's weight by roughly 2.7 to 5.4 &mdash; 100&nbsp;g of dry rice is about 280&nbsp;g cooked, and 100&nbsp;g of oats becomes a bowl of oatmeal weighing over 500&nbsp;g. Volume goes the same way (a cup of dry rice makes about three cups cooked) but the <em>cooked cup</em> is lighter than the dry one, because cooked grains are swollen and hold air between them: USDA weighs a cup of cooked white rice at 158&nbsp;g against this chart's ${g2(w("white-rice"))}&nbsp;g dry. So "cooked" makes a weight much bigger and a cup weight slightly smaller &mdash; the <a href="/dry-to-cooked/">dry to cooked converter</a> does both directions for every grain here.</p>
<p><em>Which cup?</em> A US cup is 236.6&nbsp;mL, the metric cup used in the UK, Australia and New Zealand is 250&nbsp;mL, and the rice-cooker cup that ships with Japanese cookers is the 180&nbsp;mL <em>g&#333;</em>. Same rice, three legitimate answers: about ${g2(w("white-rice"))}&nbsp;g, ${Math.round(w("white-rice") / 236.588 * 250)}&nbsp;g and ${Math.round(w("white-rice") / 236.588 * 180)}&nbsp;g. That is a ${Math.round((250 / 180 - 1) * 100)}% spread from the cup alone, before the grain is even chosen. This chart is US cups throughout.</p>
<p><em>Is it a cup weight at all?</em> Serving sizes on packaging, portion guidance and dietary-labelling figures are not measurements of a measuring cup, and they collide with cup weights constantly because both are quoted in grams.</p>
<p>Two worked examples, both taken from questions people actually arrive here with:</p>
<p><strong>&ldquo;Is there 48&nbsp;g of whole grains in half a cup of quinoa?&rdquo;</strong> Not as a weight in the cup. Half a US cup of dry quinoa is ${g2(w("quinoa") / 2)}&nbsp;g, and half a cup of <em>cooked</em> quinoa about ${g2(185 / 2)}&nbsp;g (USDA weighs the cooked cup at 185&nbsp;g). 48&nbsp;g of dry quinoa is barely more than a quarter cup &mdash; ${cups2(48 / w("quinoa"))} of a cup, or about ${g2(48 / w("quinoa") * 16)} tablespoons. The 48 comes from somewhere else entirely: the Whole Grains Council's 100% Stamp defines one whole-grain serving as 16&nbsp;g of whole grain, so 48&nbsp;g is three of those servings. It is a labelling unit for bookkeeping, not a measurement of what fits in your cup, and the two are quoted in the same unit &mdash; which is exactly why they get confused.</p>
<p><strong>&ldquo;Is a cup of rice ${g2(w("white-rice"))}&nbsp;g or 200&nbsp;g?&rdquo;</strong> Both, for different rice. This chart's ${g2(w("white-rice"))}&nbsp;g is long-grain white rice, the default in US recipes, and brown long-grain matches it. Medium-grain runs about 195&nbsp;g a cup and short-grain or sushi rice about 200&nbsp;g &mdash; the shorter, rounder kernels leave less space between them. So a 15&nbsp;g disagreement between two rice charts is usually not an error in either; it is two different rices. The <a href="/cups-to-grams/white-rice/">white rice converter</a> works through the grain lengths in full.</p>
<p class="note">One habit removes all of this: weigh the grain dry, before rinsing. Rinsed rice holds surface and absorbed water &mdash; soaking can add around 20% &mdash; so a drained cup is no longer ${g2(w("white-rice"))}&nbsp;g. Comparing two grains rather than checking one? The <a href="/ingredient-substitution-chart/">ingredient substitution chart</a> works out how much of a replacement grain to measure.</p>
<h2 id="whole-grain-serves">Whole-grain servings per cup: oats, quinoa, rice, bulgur and barley</h2>
<p>A second family of gram figures collides with cup weights: <strong>whole-grain servings</strong>. The Whole Grains Council's stamp counts <strong>16&nbsp;g of whole-grain ingredient as one serving</strong> and recommends at least three a day (48&nbsp;g), and USDA's MyPlate counts grains in <strong>ounce-equivalents</strong> &mdash; 1&nbsp;oz (${g2(OZ)}&nbsp;g) of dry rice, pasta or cereal, or 1/2 cup cooked. Both are labelling units rather than measurements, but both can be worked out from the dry cup weights on this chart with one extra fact: whether the grain is whole. Oats, quinoa and bulgur are 100% whole grain, so the grams on the scale are the grams of whole grain; white rice, pearl barley and ordinary couscous are refined, so they carry no whole-grain servings at all; cornmeal depends on the bag.</p>
<table><thead><tr><th>Grain (1 US cup, dry)</th><th>Grams</th><th>Whole grain?</th><th>Whole Grains Council servings (16&nbsp;g)</th><th>USDA ounce-equivalents (1&nbsp;oz dry)</th></tr></thead><tbody>${
  [["rolled-oats", "yes"], ["steel-cut-oats", "yes"], ["quinoa", "yes"], ["bulgur-dry", "yes"],
   ["cornmeal", "only if whole-grain / stone-ground"], ["white-rice", "no (refined)"],
   ["pearl-barley", "no (pearled)"], ["couscous-dry", "no, unless whole-wheat couscous"]]
    .map(([slug, wg]) => {
      const it = items.find((i) => i.slug === slug);
      if (!it) return "";
      const g = it.gramsPerCup, whole = wg === "yes", partial = wg.startsWith("only");
      const wgc = whole ? (g / OATS_SERVE_WGC_G).toFixed(1) : partial ? `up to ${(g / OATS_SERVE_WGC_G).toFixed(1)}` : "0";
      return `<tr><td><a href="/cups-to-grams/${slug}/">${esc(it.name)}</a></td><td class="num">${g2(g)}&nbsp;g</td><td>${wg}</td><td class="num">${wgc}</td><td class="num">${(g / OZ).toFixed(1)}</td></tr>`;
    }).join("")
}</tbody></table>
<p class="note">Cup weights come straight from the chart above; servings are that weight divided by 16&nbsp;g (whole grains only) and by ${g2(OZ)}&nbsp;g. Whole-grain status follows the Whole Grains Council's definitions: pearling strips barley's bran and degerminating removes cornmeal's germ, so neither counts, while hulled barley, whole-grain (stone-ground) cornmeal and whole-wheat couscous count in full. Brown rice weighs the same ${g2(w("white-rice"))}&nbsp;g a cup as long-grain white but is a whole grain, so a quarter cup (${g2(w("white-rice") / 4)}&nbsp;g) is nearly three 16&nbsp;g servings.</p>
<p>So <strong>half a cup of uncooked oats holds ${g2(w("rolled-oats") / 2)}&nbsp;g of whole grain</strong> (Quaker's label says 40&nbsp;g because its half-cup serving weighs 40&nbsp;g), and a cup of dry quinoa is ${(w("quinoa") / OATS_SERVE_WGC_G).toFixed(1)} whole-grain servings or ${(w("quinoa") / OZ).toFixed(1)} ounce-equivalents. The Australian and New Zealand &ldquo;standard serve&rdquo; is defined on the cooked bowl (1/2 cup of cooked rice, quinoa or porridge); the <a href="/cups-to-grams/rolled-oats/">rolled oats page</a> converts it back to dry oats, and the <a href="/dry-to-cooked/">dry to cooked converter</a> gives the cooked weight for any grain here.</p>` : ""}${key === "baking" ? `
...
<p class="note">Computed from the same verified grams-per-cup weights as the chart above (100 &divide; grams per cup), so the two tables always agree. For any other gram amount, use the <a href="/grams-to-cups/">grams to cups converter</a> &mdash; every ingredient here is in its dropdown.</p>
<h2 id="baking-fractions">Every cup fraction: chocolate chips, nuts, seeds and coconut</h2>
<p>The chart above gives the three headline amounts. This one gives every fraction a cookie or muffin recipe actually asks for &mdash; three-quarters, two-thirds, a third &mdash; plus a single tablespoon, for all ${items.length} ingredients on the page. The one everybody looks up first: <strong>half a cup of chocolate chips is ${g2(w("chocolate-chips") / 2)}&nbsp;g</strong>, a third of a cup ${g2(w("chocolate-chips") / 3)}&nbsp;g and a quarter cup ${g2(w("chocolate-chips") / 4)}&nbsp;g. Half a cup of chopped nuts is ${g2(w("chopped-nuts") / 2)}&nbsp;g, of whole almonds ${g2(w("whole-almonds") / 2)}&nbsp;g and of shredded coconut only ${g2(w("shredded-coconut") / 2)}&nbsp;g.</p>
<div class="tw"><table><thead><tr><th>Ingredient</th><th>1 cup</th><th>&frac34; cup</th><th>&frac23; cup</th><th>&frac12; cup</th><th>&frac13; cup</th><th>&frac14; cup</th><th>1 tbsp</th></tr></thead><tbody>${
  items.map((i) => `<tr><td><a href="/cups-to-grams/${i.slug}/">${esc(i.name)}</a></td>` +
    [1, 0.75, 2 / 3, 0.5, 1 / 3, 0.25, 1 / 16].map((f) => `<td class="num">${g2(i.gramsPerCup * f)} g</td>`).join("") +
    `</tr>`).join("")
}</tbody></table></div>
<p class="note">Every cell is computed from the same verified grams-per-cup weight as the chart above (&frac34; cup = weight &times; 0.75, 1 tbsp = weight &divide; 16), so the two tables can never disagree. For an amount that is not in the table, the converter at the top of the page takes any number, and <a href="/tablespoons-to-grams/">tablespoons to grams</a> handles spoon amounts.</p>
<h2 id="baking-sanity-check">Is that weight right? A ten-second sanity check for nuts, seeds and chocolate</h2>
<p>A lot of people arrive at a chart like this one not to convert a number but to <em>check</em> one &mdash; a figure from a recipe, a bag or a tracking app that looks too big or too small. Two steps settle almost all of them.</p>
<p><strong>Step 1: check the band.</strong> Baking add-ins are not scattered at random. Everything on this chart falls into one of three groups, and simply knowing which group you are in catches an implausible number immediately.</p>
<table><thead><tr><th>Group</th><th>1 cup weighs</th><th>What is in it</th></tr></thead><tbody>${
  [["Light and airy", (gr) => gr < 110],
   ["Nuts, seeds and dried fruit", (gr) => gr >= 110 && gr < 160],
   ["Chocolate", (gr) => gr >= 160]]
    .map(([label, test]) => {
      const ms = items.filter((i) => test(i.gramsPerCup)).sort((a, b) => a.gramsPerCup - b.gramsPerCup);
      return `<tr><td><strong>${label}</strong></td><td class="num">${ms[0].gramsPerCup === ms[ms.length - 1].gramsPerCup ? g2(ms[0].gramsPerCup) : `${g2(ms[0].gramsPerCup)}&ndash;${g2(ms[ms.length - 1].gramsPerCup)}`} g</td><td>${ms.map((i) => `<a href="/cups-to-grams/${i.slug}/">${esc(i.name)}</a> ${g2(i.gramsPerCup)}&nbsp;g`).join(" &middot; ")}</td></tr>`;
    }).join("")
}</tbody></table>
<p class="note">Group boundaries and every weight are read straight from the chart above &mdash; nothing here is typed by hand.</p>
<p>The middle band is where nearly everything lives, and it is narrower than people expect: <strong>every nut, seed and dried fruit on this chart sits between ${g2(w("pecan-halves"))} and ${g2(w("chia-seeds"))}&nbsp;g a cup, a ${Math.round(w("chia-seeds") / w("pecan-halves") * 10) / 10}&times; spread end to end</strong>. So an almond-sized number for a hazelnut is fine (they are both ${g2(w("whole-almonds"))}&nbsp;g); a doubled one is not. Chocolate is the heaviest thing here because chips are solid and pack tightly, and it is the one group where the spread is tiny &mdash; standard, white, mini and chunk all land within about 15&nbsp;g of ${g2(w("chocolate-chips"))}&nbsp;g. At the light end, <a href="/cups-to-grams/shredded-coconut/">shredded coconut</a> at ${g2(w("shredded-coconut"))}&nbsp;g is mostly air between the strands, so any coconut figure much above 100&nbsp;g a cup has been pressed into the cup or is for a different product.</p>
<p><strong>Step 2: check which question the number is answering.</strong> A nut or chocolate weight that fails the band test is usually not wrong, just an answer to a different question. Four possibilities, worth ruling out in this order.</p>
<p><em>Whole, halves, chopped, sliced or ground?</em> By far the most common one. The prep state moves a nut's cup weight more than the species does: USDA weighs a cup of walnut halves at ${nutG("walnut-halves")}&nbsp;g, chopped walnuts at ${nutG("walnut-chopped")}&nbsp;g and ground walnuts at ${nutG("walnut-ground")}&nbsp;g; whole almonds ${nutG("almond-whole")}&nbsp;g, slivered ${nutG("almond-slivered")}&nbsp;g, sliced ${nutG("almond-sliced")}&nbsp;g; whole hazelnuts ${nutG("hazelnut-whole")}&nbsp;g, chopped ${nutG("hazelnut-chopped")}&nbsp;g, ground ${nutG("hazelnut-ground")}&nbsp;g. That is a ${Math.round((1 - nutG("hazelnut-ground") / nutG("hazelnut-whole")) * 100)}% drop from whole to ground for the same nut, while the <em>whole</em> nuts on the list nearly all fall in a tight ${nutG("brazil")}&ndash;${nutG("peanut-raw")}&nbsp;g band (shelled pistachio kernels, at ${nutG("pistachio")}&nbsp;g, are the one outlier). This chart lists whole almonds, hazelnuts and pine nuts at the ${g2(w("whole-almonds"))}&nbsp;g baking-chart convention, inside that band; USDA's own measured cups are ${nutG("almond-whole")}, ${nutG("hazelnut-whole")} and ${nutG("pine-nut")}&nbsp;g, and a gap of a few grams is normal spread between sources for irregular nuts. And the direction is not fixed: big curved halves get <em>heavier</em> chopped (the pieces fill the gaps), compact round nuts get <em>lighter</em>. The <a href="/cups-to-grams/chopped-nuts/">chopped nuts page</a> has all 21 measured prep states.</p>
<p><em>Which chocolate?</em> Chips, minis, chunks and grated bar are different cup weights: USDA measures standard chips at 168&nbsp;g, mini chips 173&nbsp;g, chunks or large chips 182&nbsp;g, and chocolate grated from a bar only 132&nbsp;g, because shavings are mostly air. This chart's ${g2(w("chocolate-chips"))}&nbsp;g is the standard morsel. A chocolate number 15&nbsp;g either side of it is almost certainly a different piece size, not an error; the <a href="/cups-to-grams/chocolate-chips/">chocolate chips converter</a> lists every form and every US bag size.</p>
<p><em>Packed, or which cup?</em> Coconut and raisins are the two here that change with a heavy hand: shredded coconut springs and tangles, so pressing it down fits far more into the cup than the ${g2(w("shredded-coconut"))}&nbsp;g of a loose fill, and raisins clump, so a cup crushed down holds well above the ${g2(w("raisins"))}&nbsp;g of a gently settled one. This chart assumes a loose, level cup for both. Then the cup itself: a US cup is 236.6&nbsp;mL and the metric cup used in the UK, Australia and New Zealand is 250&nbsp;mL, so a metric cup of chocolate chips is about ${Math.round(w("chocolate-chips") / 236.588 * 250)}&nbsp;g and of whole almonds ${Math.round(w("whole-almonds") / 236.588 * 250)}&nbsp;g &mdash; a ${Math.round((250 / 236.588 - 1) * 100 * 10) / 10}% lift before the ingredient is even chosen. This chart is US cups throughout.</p>
<p><em>Is it a cup weight at all?</em> Nutrition labels quote grams too, and they collide with cup weights constantly. Nuts and seeds are labelled per 1&nbsp;oz (28&nbsp;g) serving &mdash; about a fifth of a cup of whole almonds (${cups2(OZ / w("whole-almonds"))} cups), a small handful, not a measure. Chocolate chips are labelled per tablespoon at 14&ndash;15&nbsp;g, and sixteen of those would be 224&ndash;240&nbsp;g, far above the ${g2(w("chocolate-chips"))}&nbsp;g a measured cup actually holds &mdash; chips cannot level in a spoon the way they settle in a cup. If a number came from a label, convert it as a weight and ignore its serving unit.</p>
<p>Two worked examples, both taken from questions people actually arrive here with:</p>
<p><strong>&ldquo;Is 145&nbsp;g of chocolate-covered peanuts a lot?&rdquo;</strong> As a quantity, it is about one cup. USDA weighs a cup of raw shelled peanuts at ${nutG("peanut-raw")}&nbsp;g and this chart's cup of chocolate chips is ${g2(w("chocolate-chips"))}&nbsp;g; a cup of chocolate-coated peanuts lands in the same region, so 145&nbsp;g is roughly a level cup &mdash; ${g2(145 / OZ)}&nbsp;oz, or about five of the 1&nbsp;oz handfuls that nut packaging calls a serving. Whether that is a lot depends on the job: folded into a batch of cookies it is an ordinary mix-in amount (a cup of chips is the standard), and eaten from the bag it is five servings.</p>
<p><strong>&ldquo;I want hazelnuts instead of &frac14; cup of mini chocolate chips &mdash; how much?&rdquo;</strong> A quarter cup of hazelnuts: about ${g2(w("hazelnuts") / 4)}&nbsp;g whole or ${g2(nutG("hazelnut-chopped") / 4)}&nbsp;g chopped, in place of ${g2(173 / 4)}&nbsp;g of minis. Mix-ins are swapped by volume, not by weight &mdash; a dough is built to carry a certain bulk of solid pieces &mdash; and because hazelnuts are lighter per cup than chocolate, matching the 43&nbsp;g by weight instead would put ${cups2(173 / w("hazelnuts"))} times the volume of nuts in the batter. The <a href="/cups-to-grams/chocolate-chips/">chocolate chips page</a> has the full swap table for every mix-in on this chart.</p>
<p class="note">One habit removes most of this: weigh nuts in the state the recipe names, after chopping if it says &ldquo;1 cup chopped nuts&rdquo; and before if it says &ldquo;1 cup nuts, chopped&rdquo;. Comparing two add-ins rather than checking one? The <a href="/ingredient-substitution-chart/">ingredient substitution chart</a> works out how much of a replacement to measure, and the <a href="/recipe-weight-calculator/">recipe weight calculator</a> totals a whole mix-in list at once.</p>` : ""}${key === "dairy" ? `
<h2 id="dairy-fractions">Every cup fraction: milk, cream, butter, yogurt, cheese and oil</h2>
<p>The chart above gives the three headline amounts. This one gives every fraction a recipe actually asks for &mdash; three-quarters, two-thirds, a third, an eighth &mdash; plus a single tablespoon, for all ${items.length} ingredients on the page. The ones people look up most: <strong>half a cup of milk is ${g2(w("milk") / 2)}&nbsp;g</strong>, half a cup of heavy cream ${g2(w("heavy-cream") / 2)}&nbsp;g, of yogurt ${g2(w("yogurt") / 2)}&nbsp;g, of grated parmesan ${g2(w("grated-parmesan") / 2)}&nbsp;g, and half a cup of butter ${g2(w("butter") / 2)}&nbsp;g &mdash; one US stick. An eighth of a cup is 2 tablespoons: ${g2(w("olive-oil") / 8)}&nbsp;g of olive oil, ${g2(w("butter") / 8)}&nbsp;g of butter.</p>
<div class="tw"><table><thead><tr><th>Ingredient</th><th>1 cup</th><th>&frac34; cup</th><th>&frac23; cup</th><th>&frac12; cup</th><th>&frac13; cup</th><th>&frac14; cup</th><th>&#8539; cup</th><th>1 tbsp</th></tr></thead><tbody>${
  items.map((i) => `<tr><td><a href="/cups-to-grams/${i.slug}/">${esc(i.name)}</a></td>` +
    [1, 0.75, 2 / 3, 0.5, 1 / 3, 0.25, 1 / 8, 1 / 16].map((f) => `<td class="num">${g2(i.gramsPerCup * f)} g</td>`).join("") +
    `</tr>`).join("")
}</tbody></table></div>
<p class="note">Every cell is computed from the same verified grams-per-cup weight as the chart above (&frac34; cup = weight &times; 0.75, 1 tbsp = weight &divide; 16), so the two tables can never disagree. For an amount that is not in the table, the converter at the top of the page takes any number; <a href="/tablespoons-to-grams/">tablespoons to grams</a> handles spoon amounts, and the <a href="/butter-converter/">butter converter</a> works in sticks.</p>
...
<h2 id="dairy-sanity-check">Is that dairy weight right? A ten-second sanity check</h2>
<p>A lot of people arrive at a chart like this one not to convert a number but to <em>check</em> one &mdash; a figure from a recipe, a carton or a tracking app that looks too big or too small. Two steps settle almost all of them.</p>
<p><strong>Step 1: check the band.</strong> Dairy and fats are not scattered at random. Everything on this chart falls into one of four groups, and simply knowing which group you are in catches an implausible number immediately.</p>
<table><thead><tr><th>Group</th><th>1 cup weighs</th><th>What is in it</th></tr></thead><tbody>${
  [["Shredded and grated cheese", (gr) => gr < 150],
   ["Oils and shortening", (gr) => gr >= 150 && gr < 225],
   ["The water band: milk, cream, butter, yogurt, soft cheese", (gr) => gr >= 225 && gr < 250],
   ["Heavier than water: nut butters, applesauce, condensed milk", (gr) => gr >= 250]]
    .map(([label, test]) => {
      const ms = items.filter((i) => test(i.gramsPerCup)).sort((a, b) => a.gramsPerCup - b.gramsPerCup);
      return `<tr><td><strong>${label}</strong></td><td class="num">${g2(ms[0].gramsPerCup)}&ndash;${g2(ms[ms.length - 1].gramsPerCup)} g</td><td>${ms.map((i) => `<a href="/cups-to-grams/${i.slug}/">${esc(i.name)}</a> ${g2(i.gramsPerCup)}&nbsp;g`).join(" &middot; ")}</td></tr>`;
    }).join("")
...
<p class="note">Group boundaries and every weight are read straight from the chart above &mdash; nothing here is typed by hand.</p>
<p>The water band is the one to remember, because nearly everything dairy lives in it. Milk, cream, buttermilk, yogurt, sour cream, cream cheese and ricotta are all mostly water by weight, and butter (four-fifths fat, but fat packed solid with no air in it) happens to match, so a cup of any of them lands within a few grams of a cup of <a href="/cups-to-grams/water/">water</a> (${g2(wAny("water"))}&nbsp;g): <strong>every pourable or spoonable dairy item here sits between ${g2(w("evaporated-milk"))} and ${g2(w("yogurt"))}&nbsp;g a cup, a spread of only ${Math.round((w("yogurt") / w("evaporated-milk") - 1) * 100)}%</strong>. So a milk-sized number for cream, or a yogurt-sized one for sour cream, is fine; a figure 20% off is not a variation, it is a different question (see step 2). Oils sit just below the band because oil is lighter than water (about 0.92&nbsp;g per mL), which is why a cup of oil is ${g2(w("olive-oil"))}&ndash;${g2(w("vegetable-oil"))}&nbsp;g and never ${g2(w("milk"))}. Shredded cheese is far lighter again &mdash; a cup is half air between the shreds. Above the band are the things that are denser than water: applesauce and the nut butters at ${g2(w("applesauce"))}&ndash;${g2(w("almond-butter"))}&nbsp;g, and <a href="/cups-to-grams/sweetened-condensed-milk/">sweetened condensed milk</a> at ${g2(w("sweetened-condensed-milk"))}&nbsp;g, about ${Math.round((w("sweetened-condensed-milk") / wAny("water") - 1) * 100)}% heavier than water because nearly half of it is dissolved sugar.</p>
<p><strong>Step 2: check which question the number is answering.</strong> A dairy weight that fails the band test is usually not wrong, just an answer to a different question. Five possibilities, worth ruling out in this order.</p>
<p><em>Fluid ounces or ounces?</em> The dairy mix-up above all others. A US cup is 8 <em>fluid</em> ounces of volume, but a cup of milk weighs ${g2(w("milk"))}&nbsp;g, which is ${g2(w("milk") / OZ)}&nbsp;oz on a scale; a cup of oil is 8&nbsp;fl&nbsp;oz but only ${g2(w("vegetable-oil") / OZ)}&nbsp;oz by weight. Butter is the one ingredient where the two ounces agree &mdash; 1 cup = ${g2(w("butter"))}&nbsp;g = ${g2(w("butter") / OZ)}&nbsp;oz = 8&nbsp;fl&nbsp;oz &mdash; and cream cheese comes close enough that an 8&nbsp;oz brick (${g2(8 * OZ)}&nbsp;g) is, for every practical purpose, the ${g2(w("cream-cheese"))}&nbsp;g measured cup. Everywhere else, "8 oz" of a liquid in a recipe means the fluid measure, a cup, and the grams follow from this chart, not from 8&nbsp;&times;&nbsp;${g2(OZ)}.</p>
<p><em>Liquid, whipped, frozen or powdered?</em> The same dairy in a different state can weigh half as much per cup. USDA measures a cup of fluid heavy whipping cream at ${DAIRY_REF.creamFluid}&nbsp;g and notes that it yields 2 cups whipped, each weighing ${DAIRY_REF.creamWhipped}&nbsp;g &mdash; whipping folds in air and adds no weight. Ice cream is the same trick frozen: USDA weighs vanilla ice cream at ${DAIRY_REF.iceCreamCup / 2}&nbsp;g per half cup, ${DAIRY_REF.iceCreamCup}&nbsp;g a cup, close to the whipped cream it resembles and half the weight of the cream it started as. Dry milk or cream <em>powder</em> is a different product again, a fraction of the liquid's cup weight and not on this chart at all. This chart lists every cream, milk and yogurt as the liquid or spoonable form.</p>
<p><em>Sticks, bricks, tubs or blocks?</em> Dairy is sold in packages that are almost, but not quite, cup-shaped. A US stick of butter is ${DAIRY_REF.butterStick}&nbsp;g, exactly half a cup; a European 250&nbsp;g block is ${cups2(250 / w("butter"))} cups, not one. An 8&nbsp;oz cream cheese brick is one cup; a 1&nbsp;lb (454&nbsp;g) tub of ricotta is ${cups2(454 / w("ricotta-cheese"))} cups on this chart's ${g2(w("ricotta-cheese"))}&nbsp;g and a little under 2 on USDA's firmer 246&nbsp;g; a 4&nbsp;oz block of cheese shreds to one cup, as the cheese table above lays out grate by grate. A single-serve yogurt pot is usually 5.3&nbsp;oz (150&nbsp;g), not a cup, and an 8&nbsp;oz pot (227&nbsp;g) comes up a heaped tablespoon short of the ${g2(w("yogurt"))}&nbsp;g measured cup.</p>
<p><em>Which cup?</em> A US cup is 236.6&nbsp;mL; the metric cup used in Australia, New Zealand, the UK and Canada is 250&nbsp;mL, ${Math.round((250 / 236.588 - 1) * 1000) / 10}% larger. Same milk, two legitimate answers: ${g2(w("milk"))}&nbsp;g and ${Math.round(w("milk") / 236.588 * 250)}&nbsp;g; same heavy cream, ${g2(w("heavy-cream"))} and ${Math.round(w("heavy-cream") / 236.588 * 250)}&nbsp;g; same coconut oil, ${g2(w("coconut-oil"))} and ${Math.round(w("coconut-oil") / 236.588 * 250)}&nbsp;g. A "half cup of heavy cream in grams, Australia" is therefore about ${Math.round(w("heavy-cream") / 236.588 * 125)}&nbsp;g, not ${g2(w("heavy-cream") / 2)}. This chart is US cups throughout; the <a href="/cups-to-ml/">cups to mL converter</a> handles the other sizes.</p>
<p><em>Is it a cup weight at all?</em> Nutrition labels quote grams too, and they collide with cup weights constantly. Oil is labelled per tablespoon at ${DAIRY_REF.oilLabelTbsp}&nbsp;g (USDA's measured tablespoon is ${DAIRY_REF.oilTbsp}&nbsp;g, rounded up on the panel), and sixteen of those is ${DAIRY_REF.oilLabelTbsp * 16}&nbsp;g against the ${g2(w("vegetable-oil"))}&nbsp;g a measured cup actually holds. Milk is labelled per 1&nbsp;cup (240&nbsp;mL) at USDA's ${DAIRY_REF.milkUsda}&nbsp;g, a little above this chart's ${g2(w("milk"))} because that cup is a rounded 240&nbsp;mL rather than 236.6. Sour cream and yogurt labels use 2&nbsp;tbsp or a container, never a cup. If a number came from a label, convert it as a weight and ignore its serving unit.</p>
<p>Two worked examples, both taken from questions people actually arrive here with:</p>
<p><strong>&ldquo;Is a cup of vegetable oil 224&nbsp;g? Mine says ${g2(w("vegetable-oil"))}.&rdquo;</strong> Both are right, and the gap has a precise cause. ${g2(w("vegetable-oil"))}&nbsp;g is the measured cup (USDA: ${DAIRY_REF.oilTbsp}&nbsp;g per tablespoon, ${g2(DAIRY_REF.oilTbsp * 16)}&nbsp;g per 16). 224&nbsp;g is the label's rounded ${DAIRY_REF.oilLabelTbsp}&nbsp;g tablespoon multiplied by 16. They differ by under 3%, which is less than the difference between filling the cup to the line and slightly over it. Either passes the band test (oils: ${g2(w("olive-oil"))}&ndash;${g2(w("vegetable-oil"))}&nbsp;g); a cup of oil at ${g2(w("milk"))}&nbsp;g or more would not, because oil cannot weigh what water does.</p>
<p><strong>&ldquo;Compare a quarter cup of butter, of ice cream and of heavy whipping cream.&rdquo;</strong> A quarter cup of butter is ${g2(w("butter") / 4)}&nbsp;g (half a stick), a quarter cup of heavy cream ${g2(w("heavy-cream") / 4)}&nbsp;g (USDA's ${DAIRY_REF.creamFluid}&nbsp;g cup gives ${g2(DAIRY_REF.creamFluid / 4)}), and a quarter cup of vanilla ice cream only ${g2(DAIRY_REF.iceCreamCup / 4)}&nbsp;g. Butter and cream tie, because both are essentially water and fat with no air in them; the ice cream weighs barely more than half, because it is churned cream that is roughly half air by volume, then frozen. The same quarter cup of <em>whipped</em> cream would be ${g2(DAIRY_REF.creamWhipped / 4)}&nbsp;g. Whenever a dairy figure comes out near half of what this chart says, look for air.</p>
<p class="note">One habit removes most of this: measure anything pourable in a liquid measuring cup at eye level, level anything spoonable with the back of a knife, and when the recipe gives a weight, skip the cup entirely. Comparing two dairy items rather than checking one? The <a href="/ingredient-substitution-chart/">ingredient substitution chart</a> works out how much of a replacement to measure, and the <a href="/recipe-weight-calculator/">recipe weight calculator</a> totals a whole list at once.</p>` : ""}
